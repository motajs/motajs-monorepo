...
`packages/apps/editor/dprint.jsonc` configures dprint (`typescript` + `json` plugins, default options) for that app. `@motajs/packer` also has an `eslint src --ext .ts` script, and the root config's `files: ["**/*.{js,ts,tsx}"]` therefore applies to it.
