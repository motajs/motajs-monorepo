...
- **D-06:** core 的 subpath 采用 **单向依赖 DAG**：`.（内核）` 不依赖任何能力 subpath；`./react`、`./shell` 只依赖 `.`；四大能力（`./table`/`./code`/`./asset`/`./map`）依赖 `.` + `./react`/`./shell`；**能力之间互不依赖**。跨 subpath 引用统一用**相对路径**（**Q1 决策，2026-09-21，修订原「包内 `@/`」**：`tsc` 的 `paths` 是 program-global——当 editor 的 `tsc -b` 把 core 源码纳入 program 时，core 的 `@/x` 会按 editor 的 `@/* → ./src/*` 解析；`editor/src/x` 不存在则 `TS2307`，存在则**静默引错文件**。`exclude` 遮不住被 import 的包源码，project references 因 core `noEmit` 报 `TS6310`。实测见 `02-RESEARCH.md` §Pattern 1）。此 DAG 直接成为 VERIFY-05 的禁止边规则（按 `lib/<dir>/` 路径表达，与相对导入无关）。 — **Reversibility:** costly — 违反/修改方向需改动 import 结构与 dependency-cruiser 规则集。
- **D-07:** PKG-03 的证明方式为 **让 editor 声明 `@motajs/editor-core` 依赖并真实 import**（探针或其导出），由现有 `pnpm typecheck`(`tsc -b`) + `pnpm build` 门禁覆盖 core 的**包内**导入解析——**Q1 决策（2026-09-21）后 PKG-03 的措辞由「core 的 `@/` 导入」改为「core 的包内导入」**，因为 D-06 已改为相对路径，`@/` 不再用于 core 内部。断言为**两极性**：`pnpm typecheck` 绿（正极性）**且**一个故意错误的 core 包内导入必须失败（负极性，`TS2307`）——负极性由 core 的 smoke 测试或一个负向 fixture 证明，**不引入独立解析工具**。这同时落地 PROJECT 约束「editor-core 作为共享库被 editor 依赖」。
...
### C. 样式与编译器跨包覆盖
- **D-09:** **editor 单点拥有 PandaCSS 配置**：`panda.config.ts` 的 `include` 扩到 `../../libs/editor-core/lib/**/*.{ts,tsx}`，CSS 单一产物仍由 editor 的 `postcss.config.cjs`（`@pandacss/dev/postcss`）产出；**core 源码 import `@styled-system/*`**，core 的 `tsconfig` `paths` 与 vitest alias 指向 editor 的 `styled-system/`（**实测更正：该目录是 gitignored，并非提交入库**；pnpm 12.5.1 会在 `pnpm install` 时执行 editor 的 `prepare: panda codegen` 重建它，四个 CI job 因此都能拿到；`tsc -b` 依赖该目录存在）。core 不新建 `panda.config.ts`（避免两份 styled-system 与 token 漂移），也符合「不新增主题/token 系统」约束。 — **Reversibility:** costly — core 因此不自包含（依赖 consumer 的生成物）；若改回 core 自带 panda 或改为共享 styled-system 包，需改动 core 内全部样式 import 与两处 tsconfig/alias。
- **D-10:** PKG-04 的断言落在 **`scripts/verify/` 下的专用验证脚本**（与 Phase 1 的 `ci-workflow.js`/`prettier-setup.js` 风格一致）：跑一次 panda 提取并断言输出 CSS 含探针的 **config 稳定静态原子 class**——**实测更正：目标是 `.display_block`，而非原先猜测的 `.d_block`**（`panda.config.ts` 的 `syntax: 'template-literal'` 令 Panda 关闭简写 `hasShorthand: false`、`prefix: undefined`，且 `hash: false`）；探针必须写成 `css\`display: block;\`` **模板字面量形式**，对象形式 `css({...})` 在该 `syntax` 下是**类型错误**。exit 0/非 0。刻意避开含动态值/自定义样式的哈希 class，避免 config 变动导致断言长期假红。
...
### E. 研究更正（2026-09-21，plan 前置）
- **D-18:** **`@douyinfe/semi-ui` 保留在 core 的 `peerDependencies` 中，但标记为 optional**（`peerDependenciesMeta: { '@douyinfe/semi-ui': { optional: true } }`）——**Q2 决策**。实测：editor 无法解析它（它是 service-worker 的 UI 库，editor 只用 antd）。因此 D-15 的 realpath 断言只覆盖 editor+core 实际消费的单例库，Semi 单独从 `service-worker` 解析并说明原因。既尊重 D-02 的 PKG-02 字面清单，又诚实编码「当前无 core 消费者」。
- **D-19:** **PKG-02 的 catalog 缺口**——实测 `antd`/`alien-signals`/`immer`/`pixi.js`/`blockly` **当前不在** `pnpm-workspace.yaml` 的 catalog 中，无法直接写 `catalog:default`；需先按 editor 现有直接 pin 的版本补齐 catalog 条目（`@douyinfe/semi-ui`、`monaco-editor` 已在 catalog）。这是对 `pnpm-workspace.yaml` 的必要改动，不改变 PKG-02 的目标。
- **D-20:** **VERIFY-05 的门禁需要两极性证明**——因为一个未解析的包内导入会让 dependency-cruiser 的边规则**静默失效**（规则永不触发也算「通过」）。因此 `core-boundaries.js` 必须：真实树 cruise → exit 0；一个合成的违规 fixture → 非零；并断言 `couldNotResolve` 未在掩盖 DAG 规则。另：`dependency-cruiser` 最新版 `18.4.0` 触发「过新」发布时效守卫，**pin `18.2.0`**；`numberOfDependentsLessThan` 只在 `forbidden`（`from` + `module`）中可用（更正 REQUIREMENTS.md 注里暗示的形状）。
- **D-21:** core 的 smoke 测试必须存在（`vitest run` 在零测试文件时非零退出，会弄红 root `pnpm test` 的 fan-out）；且探针必须是**使用 hook 的真实组件**——无 hook 的模块不会产生 react-compiler 标记。
...
- `.planning/phases/02-package-boundary-build-scaffolding/02-RESEARCH.md` — 本阶段实测研究（tsc `paths` program-global 的证明、`resolvePlugin` 插件顺序、PandaCSS 提取与类名、React Compiler 默认覆盖、dependency-cruiser 规则形状、peer/catalog 现状、13 条 pitfalls、Q1–Q5）
- `.planning/phases/02-package-boundary-build-scaffolding/02-VALIDATION.md` — 需求→测试映射（PKG-01..05、VERIFY-05）与 Wave 0 缺口清单
