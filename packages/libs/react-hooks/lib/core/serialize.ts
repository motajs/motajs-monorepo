...
export const withAsBoolean = <A extends unknown[]>(hook: (...args: A) => State<Optional<string>>) =>
  combine(hook, useAsBoolean);
...
export const withAsNumber = <A extends unknown[]>(hook: (...args: A) => State<Optional<string>>) =>
  combine(hook, useAsBoolean);
