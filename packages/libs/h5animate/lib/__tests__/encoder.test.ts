...
describe('convertObjectsToArrays', () => {
  test('应该正确转换元数据中的对象格式到数组格式', () => {
    const meta: H5AnimateMeta = {
      ...
      frame: [
        {
          objects: [{ index: 0, x: 10, y: 20, scale: 100, opacity: 255, mirror: 0, rotate: 0 }],
        },
      ],
    };
...
describe('metaReplacer', () => {
  test('应该转换 objects 数组', () => {
    const objects: H5AnimateObject[] = [{ index: 0, x: 10, y: 20, scale: 100, opacity: 255 }];
...
describe('validateEncodeInput', () => {
  ...
  test('缺少 meta 应该抛出错误', () => {
    expect(() => validateEncodeInput(null as unknown as H5AnimateMeta, validSpriteInfo, validWebpData)).toThrow(
      H5AnimateError,
    );
  });
  ...
  test('缺少 meta.ratio 应该抛出错误', () => {
    ...
    expect(() => validateEncodeInput(invalidMeta, validSpriteInfo, validWebpData)).toThrow(H5AnimateError);
  });
  ...
  test('缺少 spriteInfo 应该抛出错误', () => {
    expect(() => validateEncodeInput(validMeta, null as unknown as SpriteInfo, validWebpData)).toThrow(H5AnimateError);
  });

  test('缺少 webpData 应该抛出错误', () => {
    expect(() => validateEncodeInput(validMeta, validSpriteInfo, null as unknown as Buffer)).toThrow(H5AnimateError);
  });
