  ...
  test('包含音效数据的往返一致性', () => {
    const meta: H5AnimateMeta = {
      ...
      frame: [
        ...
        {
          sound: [{ name: 'hit.mp3' }],
          objects: [{ index: 1, x: 10, y: 10, scale: 80, opacity: 200, mirror: 1, rotate: 45 }],
        },
      ],
    };
...
describe('转换往返测试', () => {
  ...
  test('复杂动画数据的完整转换', async () => {
    ...
    const complexLegacy: LegacyAnimateFile = {
      ...
      frames: [
        ...
        [], // 空帧
        [
          [0, 20, 20, 100, 180, 0, 180],
          [1, 70, 70, 60, 150, 1, 270],
        ],
      ],
    };
  ...
  test('二次编解码的一致性', async () => {
    ...
    // 使用解码后的数据重新编码
    const secondBuffer = encodeH5Animate(firstDecoded.meta, firstDecoded.spriteInfo, firstDecoded.webpData);
