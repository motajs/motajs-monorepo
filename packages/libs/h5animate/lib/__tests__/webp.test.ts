 ...
 * 创建带透明度的测试 PNG 图像
 */
async function createTransparentPng(width: number, height: number, alpha: number): Promise<Buffer> {
  return sharp({
    create: {
      width,
      height,
...
describe('extractFrameByIndex', () => {
  ...
  test('超出范围的索引应该抛出错误', async () => {
    ...
    await expect(extractFrameByIndex(webpData, 2, { height: 50 })).rejects.toThrow(H5AnimateError);
  });
});
