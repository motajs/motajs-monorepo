...
describe('H5AnimateError', () => {
  test('应该正确创建错误实例', () => {
    const error = new H5AnimateError(H5AnimateErrorCode.INVALID_SIGNATURE, '测试错误消息');
  ...
  test('应该支持可选的位置参数', () => {
    const error = new H5AnimateError(H5AnimateErrorCode.CORRUPTED_HEADER, '文件头损坏', { position: 16 });

    expect(error.position).toBe(16);
  });

  test('应该支持可选的期望类型参数', () => {
    const error = new H5AnimateError(H5AnimateErrorCode.INVALID_SIGNATURE, '无效签名', { expectedType: 'ANIM' });

    expect(error.expectedType).toBe('ANIM');
  });

  test('应该支持可选的实际类型参数', () => {
    const error = new H5AnimateError(H5AnimateErrorCode.TYPE_MISMATCH, '类型不匹配', {
      expectedType: 'number',
      actualType: 'string',
    });
  ...
  test('应该支持可选的缺失字段参数', () => {
    const error = new H5AnimateError(H5AnimateErrorCode.VALIDATION_ERROR, '验证失败', {
  ...
  test('getFullDescription 应该返回完整的错误描述', () => {
    const error = new H5AnimateError(H5AnimateErrorCode.INVALID_SIGNATURE, '无效签名', {
      position: 0,
      expectedType: 'ANIM',
...
describe('getErrorCodeDescription', () => {
  test('应该返回正确的错误代码描述', () => {
    expect(getErrorCodeDescription(H5AnimateErrorCode.INVALID_SIGNATURE)).toContain('文件签名无效');

    expect(getErrorCodeDescription(H5AnimateErrorCode.INVALID_VERSION)).toContain('版本不受支持');

    expect(getErrorCodeDescription(H5AnimateErrorCode.TYPE_MISMATCH)).toContain('类型不匹配');

    expect(getErrorCodeDescription(H5AnimateErrorCode.MISSING_REQUIRED_FIELD)).toContain('缺少必需的字段');
  });
});
