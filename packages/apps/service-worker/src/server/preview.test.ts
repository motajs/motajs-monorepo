...
describe('project preview server', () => {
  ...
  it('serves a single byte range without caching project data', async () => {
    const response = await serveProjectPreview(
      context(new Request('https://example.test/service/1055/preview/media.bin', { headers: { range: 'bytes=1-3' } })),
    );
  ...
  it('supports HEAD with MIME and length headers', async () => {
    const response = await serveProjectPreview(
      context(new Request('https://example.test/service/1055/preview/image.png', { method: 'HEAD' }), 'image.png'),
    );
  ...
  it('returns structured permission errors for subresources', async () => {
    ...
    const response = await serveProjectPreview(
      context(new Request('https://example.test/service/1055/preview/main.js'), 'main.js'),
    );
  ...
  it('maps the memfs openAsBlob missing-file wrapper back to a structured 404', async () => {
    openAsBlob.mockRejectedValue(
      Object.assign(new TypeError('Unable to open file as blob'), { code: 'ERR_INVALID_ARG_VALUE' }),
    );
    const response = await serveProjectPreview(
      context(new Request('https://example.test/service/1055/preview/missing.png'), 'missing.png'),
    );
