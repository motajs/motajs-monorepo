...
import { EditorReleaseError, getEditorReleaseManager, resolveEditorRelease } from './editorRelease';
...
export function createEditorShell({
  ...
  const withEnvironment = html.replace(
    environmentPattern,
    (_match, open: string, close: string) => `${open}${serialized}${close}`,
  );
...
export async function serveEditorUpdateStatus(
  ...
  if (request.method === 'POST') {
    ...
    if (action === 'check') {
      const task = manager
        .checkForUpdates((body as { force?: unknown }).force === true)
...
export async function serveProjectEditor(
  ...
  const docsEndpoint = await access.fs.promises
    .stat('_docs/index.html')
