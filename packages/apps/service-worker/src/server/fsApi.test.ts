...
const formRequest = (operation: string, values: Record<string, string>, method = 'POST') =>
  new Request(`https://example.test/service/1055/api/fs/${operation}`, {
    method,
...
describe('project fs api', () => {
  ...
  it('distinguishes an empty file from a missing file', async () => {
    files.set('empty.txt', '');
    const empty = await handleFsRequest(
      1055,
      'readFile',
      formRequest('readFile', {
        name: 'empty.txt',
    ...
    const missing = await handleFsRequest(
      1055,
      'readFile',
      formRequest('readFile', {
        name: 'missing.txt',
