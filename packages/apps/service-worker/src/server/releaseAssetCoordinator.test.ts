...
describe("ReleaseAssetCoordinator", () => {
  ...
  it("reserves the fourth transfer slot for a foreground request", async () => {
    ...
    const acquire = (name: string, priority: "foreground" | "background") => coordinator.acquire({
      cacheName: "release",
    ...
    expect(fetchMock.mock.calls.map(([input]) => (input as Request).url)).toContain("https://example.test/release/page");
    expect(fetchMock.mock.calls.map(([input]) => (input as Request).url)).not.toContain("https://example.test/release/d");
