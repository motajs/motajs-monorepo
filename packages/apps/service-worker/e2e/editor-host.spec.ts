...
const projectFixtureRoot = process.env.MOTA_JS_ROOT ?? path.resolve(import.meta.dirname, '../../../external/mota-js');
...
async function writeTestEditorRelease(label: string): Promise<string> {
  const files = new Map<string, Buffer>([
    [
      'index.html',
      Buffer.from(
        `<!doctype html><html><head></head><body><div data-test-id="editor-release-marker">${label}</div><script id="mota-editor-environment" type="application/json">{"protocolVersion":1,"endpoints":{}}</script></body></html>`,
      ),
    ],
...
test('hosts the blind editor artifact against a real OPFS mota-js project', async ({ page }) => {
    ...
    .poll(() =>
      page.evaluate(async () => {
        ...
        for await (const handle of root.values())
          if (handle.kind === 'directory' && handle.name.startsWith('mota-editor-')) directories.push(handle);
  ...
  const missingRegisteredFloor = await page.evaluate(async (projectId) => {
    const request = (operation: string, values: Record<string, string>) =>
      fetch(`/service/${projectId}/api/fs/${operation}`, {
        method: 'POST',
...
test('switches Editor releases without rebuilding or replacing the active Service Worker', async ({ page }) => {
  ...
  try {
    ...
    await Promise.all(
      [releaseA, releaseB]
        .filter(Boolean)
        .map((buildId) => fs.rm(path.join(editorStaticRoot, 'releases', buildId!), { recursive: true, force: true })),
    );
  }
});
