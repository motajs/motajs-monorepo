...
async function registerOpfsProject(page: Page): Promise<{ id: number; directoryName: string }> {
  return page.evaluate(async () => {
    ...
    await write(
      'index.html',
      `<!doctype html><h1 data-test-id="preview-title">OPFS Preview</h1><script src="./main.js"></script>`,
    );
...
test('registers an OPFS project, serves preview and persists through the canonical API', async ({ page }, testInfo) => {
  ...
  if (withEditor) {
    ...
    testInfo.annotations.push({
      ...
      description:
        'MOTA_WITH_EDITOR=0: Editor hosting is intentionally out of scope for this run; no editor affordance is asserted.',
    });
  }
...
test('shows live Editor cache progress on the project page', async ({ page, editorRelease }) => {
  ...
  await expect(
    page.getByText(`Editor 构建 ${stagingBuildId.slice(0, 12)} 已缓存完成，下次打开编辑器时启用。`),
  ).toBeVisible();
});
