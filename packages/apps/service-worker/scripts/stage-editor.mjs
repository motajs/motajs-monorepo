...
export async function validateEditorArtifact(input) {
  ...
  if (manifest.buildId !== (await buildId(root, names)))
    throw new Error('Editor artifact buildId does not match its contents');
...
export async function stageEditorArtifact(input, output = path.resolve(import.meta.dirname, '../dist/static/editor')) {
  ...
  const previousBuildId =
    previousPointer?.buildId === manifest.buildId ? previousPointer?.previousBuildId : previousPointer?.buildId;
  const channel = {
    schemaVersion: 1,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const artifact = process.env.MOTA_EDITOR_ARTIFACT ?? path.resolve(import.meta.dirname, '../../editor/dist');
