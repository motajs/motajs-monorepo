...
test.describe('core panels write to sandbox project', () => {
  ...
  test('Tower music material preview starts its audio element', async ({ page }) => {
    ...
    await expect
      .poll(() => page.evaluate(() => (window as Window & { __audioPlayCalls: number }).__audioPlayCalls))
  ...
  test('Blockly field-only action blocks render on one row', async ({ page }) => {
    ...
    for (const type of ['mota_setValue_s', 'mota_showImage_s']) {
      const height = await page
        .getByTestId(`blockly-block-${type}`)
        .evaluate((node) => (node.querySelector('.blocklyPath') as SVGGraphicsElement).getBBox().height);
  ...
  test('completed legacy Blockly schemas render and persist without unknown fallback', async ({ page }) => {
    ...
    await expect(page.getByTestId('event-editor')).not.toContainText('未知事件');
    await expect(
      page.getByTestId('blockly-block-mota_setEquip_s').getByTestId('blockly-block-mota_expression'),
    ).toHaveCount(1);
  ...
  test('Floor resize uses the tile grid as the primary size editor', async ({ page }) => {
    ...
    await resizeHandle.dispatchEvent('pointermove', {
      ...
      button: 0,
      buttons: 1,
      clientX: pointerX + tileSize * 2,
      clientY: pointerY + tileSize,
    });
  ...
  test('Floor panel renames sample0 without touching the real project', async ({ page }) => {
    ...
    expect(sandbox.readText('project/floors/sample0_UI_RENAMED.js')).toContain('"floorId": "sample0_UI_RENAMED"');
  ...
  test('floorImage entry writes through the floor table', async ({ page }) => {
    ...
    expect(readFloorData(sandbox, 'sample0').images).toMatchObject([{ name: 'bg.jpg', canvas: 'bg', x: 0, y: 0 }]);
    expect(pageErrors).toEqual([]);
  });
  ...
  test('AppendPic appends and removes an item sprite without reloading material consumers', async ({ page }) => {
    ...
    await page.getByTestId('appendpic-file-input').setInputFiles(path.resolve(PROJECT_ROOT, 'materials/airwall.png'));
    ...
    await expect
      .poll(() => materialImage.evaluate((image) => (image as HTMLImageElement).naturalHeight))
  ...
  test('AppendPic appends and deletes an autotile file through the shared collection', async ({ page }) => {
    ...
    await page.getByTestId('appendpic-file-input').setInputFiles(path.resolve(PROJECT_ROOT, 'autotiles/autotile2.png'));
  ...
  test('bound images reuse the image picker without crop editing', async ({ page }) => {
    ...
    await waitForWrite;
    expect(sandbox.readText('project/enemys.js')).toMatch(
      /"greenSlime"\s*:\s*\{[^}]*"bigImage"\s*:\s*"dragon_0\.png"/s,
    );
    expect(pageErrors).toEqual([]);
  });
