...
test('only SchemaTable exposes project table customization', async ({ page }) => {
  ...
  await expect(page.getByTestId('app-topbar').getByRole('button', { name: '自定义表格' })).toHaveCount(0);
  ...
  await expect
    .poll(() =>
      page.getByTestId('code-editor-content').evaluate((element) => {
  ...
  await page.getByTestId('code-editor-content').evaluate((element) => {
    const host = element as HTMLElement & {
      __motajsMonacoEditor: { getValue(): string; setValue(value: string): void };
    };
...
test('public events follow the dark editor theme without recreating Blockly', async ({ page }) => {
  ...
  await expect
    .poll(() => blockCanvas.evaluate((element) => element.childElementCount))
...
test('Blockly builtin text fields retain contextual autocomplete', async ({ page }) => {
  ...
  await expect
    .poll(() => completions.evaluate((element) => element.getBoundingClientRect().width))
