...
function buildPasteRecord(
  ...
  const next = mode === 'merge' ? { ...cloneDeep(current), ...cloneDeep(source) } : cloneDeep(source);
  preserveTargetFields(next, current, preservedKeys(type));
  return next;
}

function propertyChanges(current: Record<string, unknown>, next: Record<string, unknown>): PrefabPropertyChange[] {
  ...
  async patch(info: PrefabInfo, actions: Action[]): Promise<CommandResult> {
    try {
      ...
      if (type === 'enemy') {
        ...
        return executePatchCommand(projectData.enemys(), prefixActions(`['${info.id}']`, actions), {
          label: `修改怪物 ${info.id}`,
          stage: 'patch-prefab',
        });
      } else if (type === 'item') {
        if (!info.id) throw new Error('Item prefab is missing id');
        return executePatchCommand(projectData.items(), prefixActions(`['${info.id}']`, actions), {
          label: `修改物品 ${info.id}`,
          stage: 'patch-prefab',
        });
      } else if (type === 'mapBlock') {
        if (info.idnum == null) throw new Error('Map block prefab is missing idnum');
        return executePatchCommand(projectData.mapBlocks(), prefixActions(`['${info.idnum}']`, actions), {
          label: `修改图块 ${info.idnum}`,
          stage: 'patch-prefab',
        });
      } else {
