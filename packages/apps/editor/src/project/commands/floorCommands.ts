...
function createInitialFloorData(
  ...
  const emptyMap = Array.from({ length: height }, () => Array.from({ length: width }, () => 0));
...
function resizeMap(currentMap: unknown, width: number, height: number, offsetX: number, offsetY: number): unknown[][] {
...
class FloorCommands {
  ...
  async batchPatch(changes: BatchFloorPatch[]): Promise<CommandResult> {
    return executeCompositeCommand(
      changes.map((change) =>
        patchResourceOperation(projectData.floor(change.floorId), change.actions, {
          label: `批量修改楼层`,
          stage: `batch-patch-floor:${change.floorId}`,
        }),
      ),
    ...
    try {
      ...
      return executeCompositeCommand(
        [
          writeTextFileOperation(
            path,
            serializeToJsMapFile(targetFloorId, copied),
            {
    ...
    try {
      ...
      return executeCompositeCommand(
        [
          writeTextFileOperation(
            path,
            serializeToJsMapFile(floorId, floorData),
            {
              label: `新建楼层 ${floorId}`,
              stage: 'write-new-floor',
            },
  ...
  async batchCreate(floors: BatchCreateFloorOptions[]): Promise<CommandResult> {
    ...
    try {
      const tower = projectData.tower().value();
      const layers = getMapLayerSettingsSnapshot();
      const operations: EditorOperation<unknown>[] = floors.map((floor) =>
        writeTextFileOperation(
      ...
      operations.push(
        patchResourceOperation(projectData.tower(), towerActions, {
          label: '批量新建楼层',
          stage: 'batch-create-floor:update-floorIds',
        }),
      );
      operations.push(
        navigateFloorOperation(floors[0].floorId, {
          label: '批量新建楼层',
          stage: 'batch-create-floor:navigate',
        }),
      );
      return executeCompositeCommand(operations, {
        label: `批量新建 ${floors.length} 个楼层`,
        stage: 'batch-create-floors',
      });
    } catch (error) {
    ...
    try {
      ...
      return executeCompositeCommand(
        [
          writeTextFileOperation(
            newPath,
            serializeToJsMapFile(newFloorId, newData),
            {
              label: `重命名楼层 ${oldFloorId}`,
              stage: 'write-renamed-floor',
            },
          ...
          navigateFloorOperation(
            ...
            {
              label: `重命名楼层 ${oldFloorId}`,
              stage: 'navigate-renamed-floor',
            },
            oldFloorId,
          ),
          deleteTextFileOperation(
            floorPath(oldFloorId),
            {
              label: `重命名楼层 ${oldFloorId}`,
              stage: 'delete-old-floor',
            },
  ...
  async resize(floorId: string, options: ResizeFloorOptions): Promise<CommandResult> {
    ...
    try {
      ...
      const coordinatePlan = await buildFloorCoordinateTransformPlan(floorId, ([x, y]) => {
        ...
        return nextX >= 0 && nextX < width && nextY >= 0 && nextY < height ? [nextX, nextY] : null;
      });
      ...
      const operations: EditorOperation<unknown>[] = [
        patchResourceOperation(projectData.floor(floorId), actions, {
          label: `调整楼层尺寸 ${floorId}`,
          stage: 'resize-floor',
        }),
      ];
      operations.push(
        ...coordinatePlan.patches.map((patch) =>
          patchResourceOperation(patch.resource, patch.actions, { label: patch.label, stage: patch.stage }),
        ),
      );
      return executeCompositeCommand(operations, {
        label: `调整楼层尺寸 ${floorId}`,
