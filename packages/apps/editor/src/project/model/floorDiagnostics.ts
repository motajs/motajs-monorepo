...
function validateSize(diagnostics: Diagnostic[], key: 'width' | 'height', value: unknown): void {
...
function validatePoint(diagnostics: Diagnostic[], floor: FloorData, key: 'upFloor' | 'downFloor' | 'flyPoint'): void {
...
export function buildFloorDiagnostics(floor: FloorData, selectedFloorId: string): Diagnostic[] {
  ...
  if (!isValidFloorId(floor.floorId)) {
    diagnostics.push(
      diagnostic('floor:floorId', 'floor.id.invalid', 'floorId 只能包含字母、数字和下划线，且不能以数字开头'),
    );
    ...
    diagnostics.push(
      diagnostic(
        'floor:floorId',
        'floor.id.mismatch',
        `数据中的 floorId 与当前楼层 ${selectedFloorId} 不一致`,
        'warning',
      ),
    );
  }
  ...
  if (floor.weather != null) {
    ...
    if (!Array.isArray(weather) || weather.length !== 2 || typeof weather[0] !== 'string' || weather[0].length === 0) {
