...
export function buildTowerDiagnostics(tower: TowerData): Diagnostic[] {
  ...
  const hero = firstData.hero && typeof firstData.hero === "object"
  ...
  if (typeof firstData.name !== "string" || !/^[A-Za-z0-9_]{1,30}$/.test(firstData.name)) {
    diagnostics.push(diagnostic(
      "tower:firstData.name",
      "tower.name.invalid",
      "工程标识符必须是 1 到 30 位字母、数字或下划线。",
    ));
  }

  if (!Array.isArray(tower.main.floorIds) || !tower.main.floorIds.includes(tower.firstData.floorId)) {
    diagnostics.push(diagnostic(
      "tower:firstData.floorId",
      "tower.initial-floor.invalid",
      "初始楼层不在当前楼层列表中。",
    ));
  }
  ...
  const numericSources: Array<[string, unknown]> = [
    ...
    ...["hpmax", "hp", "manamax", "mana", "atk", "def", "mdef", "money", "exp"]
      .map((key) => [`tower:firstData.hero.${key}`, hero[key]] as [string, unknown]),
  ];
