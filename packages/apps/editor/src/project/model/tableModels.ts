...
function safeValue(node: Node | undefined, env: Record<string, unknown>): unknown {
  ...
  if (node.type === 'ObjectExpression')
    return Object.fromEntries(
      node.properties.flatMap((property: Node) => {
  ...
  if (node.type === 'MemberExpression') {
    ...
    return object && typeof object === 'object'
      ? (object as Record<PropertyKey, unknown>)[property as PropertyKey]
      : undefined;
  }
...
export function buildEnemySpecialCatalog(functions: Record<string, unknown>): EnemySpecialCatalog {
  ...
  if (!returned || returned.type !== 'ArrayExpression') {
    return {
      entries: [],
      diagnostics: [
        { source: 'enemy-specials', severity: 'warning', message: 'Cannot statically parse enemys.getSpecials' },
      ],
    };
  }
...
export function projectImageCatalogContent(): Content<ProjectImageCatalog> {
  ...
  const physicalNames =
    directory.status === 'loaded' ? directory.value.entries.filter((name) => /\.(png|jpe?g|gif)$/i.test(name)) : [];
  ...
  for (const definition of splitDefinitions(towerValue)) {
    ...
    if (source.status !== 'loaded') {
      diagnostics.push({
        source: `project-images:${sourceName}`,
        severity: 'warning',
        message: `Split image source ${sourceName} is unavailable`,
      });
      continue;
    }
    ...
    if (!size) {
      diagnostics.push({
        source: `project-images:${sourceName}`,
        severity: 'warning',
        message: `Split image source ${sourceName} is not a PNG`,
      });
      continue;
    }
    let index = 0;
    for (let y = 0; y < size.height; y += height) {
      for (let x = 0; x < size.width; x += width) {
        ...
        if (names.has(name)) {
          diagnostics.push({
            source: `project-images:${name}`,
            severity: 'warning',
            message: `Logical image ${name} conflicts with another image`,
          });
          continue;
        }
        names.add(name);
...
export function createProjectImageResource(): ModelResource<ProjectImageCatalog> {
  const dependencies = () => {
    ...
    const sources = splitDefinitions(towerContent.value as unknown as Record<string, unknown>).flatMap((definition) =>
  ...
  return computedResource('projectImageCatalog', dependencies, projectImageCatalogContent, reloadDependencies);
}
...
export function createTableSchemaResource(
  ...
  return computedResource(
    ...
    () => {
      ...
      const towerValue =
        tower.status === 'loaded' ? tower.value : { main: { floorIds: [] }, firstData: { floorId: '' } };
      const main = towerValue.main as Record<string, unknown>;
      const dependencyDiagnostics: ProjectDiagnostic[] = [];
      if (tower.status === 'error')
        dependencyDiagnostics.push({ source: 'table-schema:tower', severity: 'warning', message: tower.error.message });
      if (tower.status === 'not-found')
        dependencyDiagnostics.push({
          source: 'table-schema:tower',
          severity: 'warning',
          message: 'project/data.js is unavailable',
        });
      const context = createTableMetaRuntimeContext({
        ...
        images: {
          ...
          tilesets: Array.isArray(main.tilesets)
            ? main.tilesets.filter((name): name is string => typeof name === 'string')
            : [],
          animates: Array.isArray(main.animates)
            ? main.animates.filter((name): name is string => typeof name === 'string')
            : [],
          bgms: Array.isArray(main.bgms) ? main.bgms.filter((name): name is string => typeof name === 'string') : [],
          sounds: Array.isArray(main.sounds)
            ? main.sounds.filter((name): name is string => typeof name === 'string')
            : [],
        },
