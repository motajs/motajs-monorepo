...
describe("raster material operations", () => {
  it("appends, replaces, crops and removes rows without changing other pixels", () => {
    ...
    expect([...inserted.rowRemap.entries()]).toEqual([[0, 0], [1, 2]]);
  });
...
describe("ProjectAssets", () => {
  ...
  it("undoes and redoes sprite append, replace and middle-row removal with row payloads", async () => {
    ...
    const appended = await operationHistory.execute(appendMaterialOperation(
      collection,
      raster(32, 32, 2),
      undefined,
      { label: "append", stage: "append" },
    ));
    expect(collection.entries()).toHaveLength(4);
    ...
    operationHistory.clear();
    await operationHistory.execute(removeMaterialOperation(
      collection,
      collection.entries()[1],
      { label: "remove", stage: "remove" },
    ));
    expect(collection.entries()).toHaveLength(3);
  ...
  it("undoes and redoes autotile file creation and deletion", async () => {
    ...
    const mutation = await operationHistory.execute(appendMaterialOperation(
      collection,
      raster(96, 128, 2),
    ...
    await operationHistory.execute(removeMaterialOperation(
      collection,
      mutation.entry!,
      { label: "remove autotile", stage: "remove-autotile" },
    ));
    expect(memory.hasFile("project/autotiles/historyAutotile.png")).toBe(false);
    await operationHistory.undo();
    expect(memory.hasFile("project/autotiles/historyAutotile.png")).toBe(true);
    expect((await collection.read(collection.entries().find((entry) => entry.key.includes("historyAutotile"))!)).data[0]).toBe(2);
    operationHistory.clear();
  });
});
