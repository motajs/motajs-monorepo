...
import { compositeOperation, patchResourceOperation, type EditorOperation, type OperationMeta } from './operations';
...
export async function executeCompositeCommand(
  ...
  try {
    ...
    const stage =
      error && typeof error === 'object' && 'commandStage' in error ? String(error.commandStage) : options.stage;
...
export async function executePatchCommand<T>(
  ...
  try {
    await operationHistory.execute(patchResourceOperation(resource, actions, options));
    return commandOk();
  } catch (error) {
