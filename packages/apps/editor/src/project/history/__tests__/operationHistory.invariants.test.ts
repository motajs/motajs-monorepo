...
describe('operationHistory invariants', () => {
  ...
  function failingOperation(targets: readonly OperationTarget[], mutate: () => void): EditorOperation {
    return {
  ...
  it('captures and restores each distinct target once, de-duplicated by key', async () => {
    ...
    expect([...tracked.captures.entries()]).toEqual([
      ['a', 1],
      ['b', 1],
    ]);
    expect([...tracked.restores.entries()]).toEqual([
      ['b', 1],
      ['a', 1],
    ]);
  ...
  it('restores distinct targets in the reverse of their capture order', async () => {
    const tracked = createTrackedTargets();
    const operation = failingOperation([tracked.target('a'), tracked.target('b'), tracked.target('c')], () => {
    ...
    expect(tracked.log.join('|')).toBe('capture:a|capture:b|capture:c|restore:c|restore:b|restore:a');
  });

  it('leaves every target at its pre-apply value and records no entry when apply fails', async () => {
    const tracked = createTrackedTargets();
    const operation = failingOperation([tracked.target('a'), tracked.target('b')], () => {
      tracked.state.set('a', 10);
      tracked.state.set('b', 20);
    });
