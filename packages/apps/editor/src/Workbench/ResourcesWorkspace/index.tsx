...
type ResourcePageId =
  | "images" | "bgms" | "sounds" | "animates" | "fonts" | "tilesets"
  | "autotiles" | "materials" | "append";
...
const DIRECTORIES: Record<string, DirectoryDescriptor> = {
  images: { id: "images", label: "图片", path: "project/images", registry: "images", accept: (name) => /\.(png|jpe?g|gif)$/i.test(name), kind: "image" },
  bgms: { id: "bgms", label: "背景音乐", path: "project/bgms", registry: "bgms", accept: (name) => /\.(mp3|ogg|wav|m4a|flac)$/i.test(name), kind: "audio" },
  sounds: { id: "sounds", label: "音效", path: "project/sounds", registry: "sounds", accept: (name) => /\.(mp3|ogg|wav|m4a|flac)$/i.test(name), kind: "audio" },
  animates: {
    id: "animates", label: "动画", path: "project/animates", registry: "animates",
    accept: (name) => name.endsWith(".animate"),
    toRegistry: (name) => name.replace(/\.animate$/i, ""),
    fromRegistry: (name) => `${name}.animate`, kind: "animation",
  },
  fonts: {
    id: "fonts", label: "字体", path: "project/fonts", registry: "fonts",
    accept: (name) => /\.ttf$/i.test(name),
    toRegistry: (name) => name.replace(/\.ttf$/i, ""),
    fromRegistry: (name) => `${name}.ttf`, kind: "font",
  },
  tilesets: { id: "tilesets", label: "瓦片图集", path: "project/tilesets", registry: "tilesets", accept: (name) => /\.png$/i.test(name), kind: "image" },
  autotiles: {
    id: "autotiles", label: "自动元件", path: "project/autotiles",
    accept: (name) => /\.png$/i.test(name),
    toRegistry: (name) => name.replace(/\.png$/i, ""),
    fromRegistry: (name) => `${name}.png`,
    kind: "image",
  },
  materials: { id: "materials", label: "系统图块", path: "project/materials", accept: (name) => /\.png$/i.test(name), kind: "image" },
};
...
const ImagePreview: FC<{ path: string; name: string; compact?: boolean }> = ({ path, name, compact }) => {
  ...
  return url ? (
    ...
    />
  ) : <div className="resourcePreviewPlaceholder"><FileImage size={compact ? 22 : 38} /></div>;
};
...
const SplitImageDialog: FC<{
  ...
  const valid = Number.isInteger(width) && Number(width) > 0
  ...
  return (
    ...
    >
      <div className="splitImageForm">
        <label><span>小图宽度</span><InputNumber min={1} precision={0} value={width} onChange={setWidth} /></label>
        <label><span>小图高度</span><InputNumber min={1} precision={0} value={height} onChange={setHeight} /></label>
...
const ResourceDetail: FC<{
  ...
  return (
    <div className="resourceDetail">
      <h2>{name}</h2>
      <p>{path}</p>
      {splitEntry ? <div className="resourceLogicalNotice">逻辑切分图片 · {splitEntry.crop?.width} × {splitEntry.crop?.height}</div> : null}
      ...
      {!missing && descriptor.kind === "font" ? (
        <div className="resourceFontSample">魔塔 Mota 0123456789</div>
      ) : null}
      {!missing && descriptor.kind === "animation" ? (
        <div className="resourcePreviewPlaceholder"><FileArchive size={42} /><span>动画文件</span></div>
      ) : null}
      {descriptor.id === "images" && !missing && !splitEntry && onConfigureSplit ? (
        <div className="resourceDetailActions">
          <button type="button" onClick={onConfigureSplit}>{splitDefinition ? "编辑切分" : "创建切分"}</button>
          {splitDefinition && onRemoveSplit ? <button type="button" className="danger" onClick={onRemoveSplit}>移除切分</button> : null}
        </div>
      ) : null}
    </div>
  );
...
const PersistentResourceDetail: FC<{
  ...
  const directory = useMemo(
    () => projectAssets.directory(selection.descriptor.path),
    [selection.descriptor.path],
  );
  ...
  const exists = selection.splitEntry || content.status !== "loaded"
    || content.value.entries.includes(selection.name);
  useEffect(() => {
    if (!exists) onRemoved();
  }, [exists, onRemoved]);
  ...
  return (
    <ResourceDetail
      ...
      name={selection.name}
      splitEntry={selection.splitEntry}
    />
  );
};
      ...
      {values.map((name) => (
        <div
          ...
          onDrop={(event) => {
            ...
            void confirmRisk("调整瓦片图集顺序？", "这会改变已有 tileset 图块的数字 ID 映射。")
              .then((confirmed) => confirmed ? onChange(next) : undefined);
          }}
        ><GripVertical size={15} />{name}</div>
      ))}
    </section>
  );
};
...
const DirectoryPage: FC<{
  ...
  const files = content.status === "loaded"
    ? content.value.entries.filter(descriptor.accept)
    : [];
  const genericRegistered = descriptor.registry && Array.isArray(tower.main[descriptor.registry])
    ? (tower.main[descriptor.registry] as unknown[]).filter((item): item is string => typeof item === "string")
    : [];
  const registered = descriptor.id === "autotiles"
  ...
  const logicalEntries = descriptor.id === "images" && imageCatalogContent.status === "loaded"
  ...
  const displayEntries = physicalAndMissingFiles.flatMap((name) => {
    ...
    return [
      ...
      ...(openSplitFolders.has(name)
        ? children.map((entry) => ({ kind: "file" as const, name: entry.name }))
        : []),
    ];
  });
  const selectedStillExists = !selected || displayEntries.some((entry) => (
    entry.kind === "file" && entry.name === selected
  ));
  useEffect(() => {
    if (!selectedStillExists) onSelect(undefined);
  }, [onSelect, selectedStillExists]);

  const enqueueRegistryMutation = useCallback(async (mutate: (current: string[]) => string[]) => {
    ...
    const run = registryQueueRef.current.then(async () => {
      ...
      const result = await tableCommands.patchTower([
        ["change", `['main']['${String(registry)}']`, next],
      ]);
  ...
  const writeRegistry = useCallback(async (next: string[]) => {
  ...
  const toggle = async (file: string, nextChecked: boolean) => {
    ...
    await mutateRegistry((current) => nextChecked
  ...
  return (
    <div className="resourceBrowser">
      <section className="resourceGridPane">
        <header><h1>{descriptor.label}</h1><span>{files.length} 个文件</span></header>
            ...
            if (display.kind === "split-folder") {
              ...
              return (
                <div
                  ...
                  onClick={() => setOpenSplitFolders((current) => {
              ...
              >
                <span className="resourceCardPreview">
                  {!missing && splitEntry
                    ...
                    : descriptor.kind === "audio" ? <FileAudio size={25} /> : <FileText size={25} />}
                </span>
                <span className="resourceCardName" title={name}>{name}</span>
      ...
      <aside className="resourceDetailPane">
        {selection && selection.descriptor.id !== descriptor.id ? (
          ...
          <ResourceDetail
            ...
            onConfigureSplit={selected && files.includes(selected) && /\.png$/i.test(selected)
              ? () => setSplitEditor(selectedDefinition ?? "new")
              : undefined}
            onRemoveSplit={selectedDefinition ? () => {
              void confirmRisk("移除图片切分？", `将同时移除由 ${selectedDefinition.name} 生成的逻辑图片。`).then(async (confirmed) => {
                if (!confirmed) return;
                const next = definitions.filter((entry) => entry.name !== selectedDefinition.name);
                notifyCommandResult(await tableCommands.patchTower([["change", "['main']['splitImages']", next]]), "图片切分已移除");
              });
            } : undefined}
          />
        )}
