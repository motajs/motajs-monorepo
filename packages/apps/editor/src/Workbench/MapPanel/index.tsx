import { useCallback, useEffect, useMemo, useRef, useState, type DragEvent, type FC } from 'react';
import { Button, Dropdown, Input, Modal, Tabs, type MenuProps } from 'antd';
import { AlertTriangle, Copy, FilePlus2, MoreHorizontal, Plus, Search } from 'lucide-react';
...
export const MapPanel: FC = () => {
  ...
  const createPartition = useCallback(
    async (targetFloorId: string) => {
      if (
        !organization.valid ||
        partitionContainingFloor(tower.main.floorIds, organization.partitions, targetFloorId) != null
      )
        return;
      const partitions = [...organization.partitions, [targetFloorId, targetFloorId] as FloorPartition].sort(
        ([left], [right]) => tower.main.floorIds.indexOf(left) - tower.main.floorIds.indexOf(right),
      );
  ...
  const floorMenu = useCallback(
    (targetFloorId: string, exists: boolean): MenuProps => {
      ...
      return {
        items: [
          ...
          {
            key: 'copy',
            label: <span data-test-id={`floor-copy-${targetFloorId}`}>复制地图…</span>,
            icon: <Copy size={14} />,
            disabled: !exists || !organization.valid,
          },
          {
            key: 'copy-blank',
            label: <span data-test-id={`floor-copy-blank-${targetFloorId}`}>复制空白地图…</span>,
          ...
          {
            key: 'clear',
            label: <span data-test-id="map-clear-submit">清空地图…</span>,
            danger: true,
            disabled: !exists,
          },
          { type: 'divider' },
  ...
  const renderFloor = (one: FloorListItem, tokenIndex?: number) => {
    ...
    const row = (
      ...
      >
        <button type="button" className="floorManagementSelect" onClick={() => one.exists && navigateFloor(one.id)}>
          ...
          <span>
            {one.id}
            {one.name && one.name !== one.title ? ` · ${one.name}` : ''}
          </span>
        </button>
        {!one.exists ? (
          <span className="floorManagementMissing" title="楼层文件不存在">
        ...
        <Dropdown menu={menu} trigger={['click']}>
          <button
            type="button"
            className="floorManagementMore"
            data-test-id={`floor-actions-${one.id}`}
            aria-label={`${one.id} 更多操作`}
    ...
    return (
      <Dropdown key={one.id} menu={menu} trigger={['contextMenu']}>
        {row}
      </Dropdown>
    );
  };
  ...
  const renderToken = (token: FloorOrganizationToken, index: number) => {
    if (token.kind === 'floor')
      return renderFloor(floorById.get(token.floorId) ?? { id: token.floorId, exists: false }, index);
    ...
    return (
      <Dropdown
        ...
        trigger={['contextMenu']}
        menu={{
          items: [{ key: 'delete', label: '删除分区', danger: true }],
          onClick: () => removePartition(token.partition),
        }}
      >
        <div
  ...
  const applyRepair = async () => {
    try {
      ...
      const result = await floorCommands.updateOrganization({
        floorIds: [...tower.main.floorIds],
        floorPartitions: validation.partitions,
      });
    ...
    <div id="left" className="leftTab floorManagementPanel" data-test-id="panel-map">
      ...
      {!organization.valid ? (
        <div className="floorOrganizationDiagnostic" data-test-id="floor-organization-diagnostic">
          ...
          <div>
            <strong>楼层分区数据异常</strong>
            <span>{organization.diagnostics.join('；')}</span>
          </div>
          <Button
            size="small"
            onClick={() => {
      ...
      {filter.trim() ? (
        <p className="floorManagementFilterHint">筛选结果仅供选择；清空搜索后可调整顺序和分区。</p>
      ) : null}
      <div className="floorManagementList" data-test-id="floor-management-list">
        {filter.trim() ? filteredFloors.map((floor) => renderFloor(floor)) : tokens.map(renderToken)}
        {filter.trim() && filteredFloors.length === 0 ? (
          <div className="floorManagementEmpty">没有匹配的楼层</div>
        ) : null}
              ...
              children: (
                <div className="mapCreateForm" data-test-id="map-create-form">
                  <label>
                    ...
                    <Input
                      value={newFloorId}
                      onChange={(event) => setNewFloorId(event.target.value)}
                  ...
                  <label>
                    ...
                    <Input
                      ...
                      onChange={(event) => setNewFloorTitle(event.target.value)}
                      data-test-id="map-create-title"
                    />
                  </label>
                  <label>
                    状态栏名称
                  ...
                  <Button type="primary" onClick={() => void submitSingleCreate()} data-test-id="map-create-submit">
                    创建楼层
                  </Button>
                </div>
              ),
      ...
      >
        ...
        <label className="mapDialogField">
          新楼层 ID
          <Input
            autoFocus
            value={copyFloorId}
            onChange={(event) => setCopyFloorId(event.target.value)}
            data-test-id="map-copy-id"
          />
        </label>
      </Modal>
      ...
      <Modal
        ...
        footer={
          dataDialog?.mode === 'export'
            ? [
                <Button key="cancel" onClick={() => setDataDialog(undefined)}>
                  关闭
                </Button>,
                <Button
                  key="copy"
                  type="primary"
                  onClick={() => void copyText(mapText).then(() => notifySuccess('地图数据已复制'))}
                >
                  复制
                </Button>,
              ]
            : [
                ...
                <Button
                  key="apply"
                  type="primary"
                  data-test-id="map-import-submit"
                  onClick={() => void applyMapImport()}
                >
                  应用导入
                </Button>,
              ]
      ...
      <Modal
        ...
        onOk={() => void applyRepair()}
        okText="校验并保存"
      >
        <p>
          请输入由 <code>[起始楼层, 结束楼层]</code> 组成、按楼层顺序排列且互不重叠的数组。
        </p>
        <Input.TextArea
          rows={10}
          value={repairText}
          onChange={(event) => setRepairText(event.target.value)}
          data-test-id="floor-partition-repair-json"
        />
      </Modal>
    </div>
  );
