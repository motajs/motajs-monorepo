...
export const BatchCreateMapsForm: FC<BatchCreateMapsFormProps> = (props) => {
  ...
  return (
    <div className="mapBatchCreateForm" data-test-id="map-batch-form">
      <label>楼层 ID 模板<Input data-test-id="map-batch-floor-ids" value={newFloorIds} onChange={(event) => setNewFloorIds(event.target.value)} /></label>
      <label>楼层标题模板<Input data-test-id="map-batch-floor-titles" value={newFloorTitles} onChange={(event) => setNewFloorTitles(event.target.value)} /></label>
      <label>状态栏名称模板<Input data-test-id="map-batch-floor-names" value={newFloorNames} onChange={(event) => setNewFloorNames(event.target.value)} /></label>
      <div className="mapBatchRange">
        <label>起始 i<Input data-test-id="map-batch-from" value={newMapsFrom} onChange={(event) => setNewMapsFrom(event.target.value)} /></label>
        <span>至</span>
        <label>结束 i<Input data-test-id="map-batch-to" value={newMapsTo} onChange={(event) => setNewMapsTo(event.target.value)} /></label>
      </div>
      <div className="mapCreateDimensions">
        <label>宽<Input data-test-id="map-batch-width" value={newMapsWidth} onChange={(event) => setNewMapsWidth(event.target.value)} /></label>
        <span>×</span>
        <label>高<Input data-test-id="map-batch-height" value={newMapsHeight} onChange={(event) => setNewMapsHeight(event.target.value)} /></label>
      </div>
      <p className="mapBatchTemplateHint">模板支持 <code>{"${i}"}</code> 及简单算术表达式；每次最多创建 99 层。</p>
      <Button type="primary" onClick={() => void createNewMaps()} data-test-id="map-batch-submit">创建楼层</Button>
    </div>
  );
};
