...
export const CommonEventsWorkspace: FC = () => {
  ...
  const activeSelected = selected && Object.prototype.hasOwnProperty.call(events, selected)
    ? selected
    : names[0] ?? "";
    ...
    editor.open({
      ...
      onDraftChange: (value) => {
        ...
        if (typeof value === "string") {
          try { normalized = JSON5.parse(value); } catch { /* 保留尚未解析的源码草稿。 */ }
        }
        setDrafts((current) => {
      ...
      onConfirm: async (value) => {
        const result = await tableCommands.patchCommonEvents([
          ["change", buildFieldPath([activeSelected]), value],
        ]);
  ...
  return (
    <div className="commonEventsWorkspace" data-test-id="common-events-workspace">
      <aside className="commonEventList">
        <header><h1>公共事件</h1><button aria-label="新增公共事件" onClick={() => void add()}><Plus size={16} /></button></header>
        {names.map((name) => (
      ...
      <main className="commonEventEditorPane">
        {activeSelected ? <div id="common-event-editor-host" /> : <div className="commonEventEmpty">新建或选择一个公共事件</div>}
      </main>
    </div>
  );
};
