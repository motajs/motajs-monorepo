...
import { getLocDataFromFloor, type LocData, type LocTarget } from '@/project/model/locModel';
...
export const LocTable: FC<LocTableProps> = ({ target, meta, editMode }) => {
  ...
  const locData = useMemo(() => getLocDataFromFloor(floorData, target.pos), [floorData, target.pos]);
  ...
  return (
    <div id="locTable" data-test-id="loc-table">
      <section data-test-id="loc-summary" style={{ margin: '8px 15px 10px', fontSize: 13, lineHeight: 1.8 }}>
        <div>
          <strong>位置</strong>{' '}
        ...
        {changeFloor ? <div data-test-id="loc-change-floor-summary">楼层切换: {changeFloor}</div> : null}
      </section>
      <Table data={locData} commentObj={meta} onChange={handleChange} editMode={editMode} />
    </div>
  );
};
