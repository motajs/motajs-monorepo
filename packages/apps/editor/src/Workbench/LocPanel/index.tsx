...
const LocSummary: FC<{ target: LocTarget; locData: LocData }> = ({ target, locData }) => {
  const changeFloor =
    locData.changeFloor && typeof locData.changeFloor === 'object' && !Array.isArray(locData.changeFloor)
  ...
  return (
    <section className="locSchemaSummary" data-test-id="loc-summary">
      <div>
        <strong>位置</strong>{' '}
        <span data-test-id="loc-selected-position">
...
const LocSchemaTable: FC<{ target: LocTarget }> = ({ target }) => {
  ...
  const block = rawBlockIdnum === 0 && typeof floor.defaultGround === 'string' ? floor.defaultGround : rawBlockIdnum;
  const scope = useMemo<SchemaScope>(() => {
    ...
    const writeLoc = (path: readonly string[], slot: ReferenceUpdate['slot']) => writeLocBatch([{ path, slot }]);
    return {
      roots: {
  ...
  return (
    <>
      ...
      <ProjectSchemaTable definition={locSchemaDefinition} scope={scope} diagnostics={diagnostics} />
    </>
  );
};
...
const LocPanelContent: FC<LocPanelContentProps> = ({ editMode, floorId, tableVersion }) => {
  ...
  const target = useMemo(
    () => resolveLocTarget(selection && floorId ? { ...selection, floorId } : selection, floorId),
    [selection, floorId],
  );
