...
const MonacoSurface = forwardRef<
  MonacoSurfaceHandle,
  {
...
>(({ activeDocumentId, documents, onChange, onDiagnostics, onSave, fontSize, fontBold }, ref) => {
  ...
  const mount = useCallback(
    (editor: MonacoEditorInstance) => {
      ...
      const surfaceNode = domNode?.closest('.scriptCodeEditor') as
        (HTMLElement & { __motajsMonacoEditor?: MonacoEditorInstance }) | null;
...
const ScriptTree: FC<{
  ...
  const renderObject = (object: ScriptDataObject, prefix: string[], depth: number) =>
    Object.entries(object).map(([key, child]) => {
      ...
      if (typeof child === 'string') {
        return (
          <button
            className="scriptTreeLeaf"
            key={id}
            onClick={() => onOpen({ path, source: child })}
      ...
      return (
        <div key={id}>
          <button
            className="scriptTreeGroup"
            onClick={() => setCollapsed((current) => ({ ...current, [id]: !closed }))}
  ...
  return (
    <div className="scriptTree" data-kind={kind}>
      {renderObject(data, [], 0)}
    </div>
  );
};
...
export const ScriptsWorkspace: FC = () => {
  ...
  const canPreviewStatusBar = active?.kind === 'functions' && active.path.join('.') === 'ui.drawStatusBar';
  ...
  const updateDocument = useCallback((documentId: string, text: string) => {
    setTabs((current) =>
      current.map((tab) =>
        tab.id === documentId ? { ...tab, text, dirty: text !== tab.base, validation: undefined } : tab,
      ),
    );
  }, []);
  ...
  const updateDiagnostics = useCallback((documentId: string, diagnostics: ScriptDiagnostic[]) => {
    setTabs((current) => {
      ...
      const next = current.map((tab) => {
        if (tab.id !== documentId || (tab.validation && diagnosticsEqual(tab.validation.diagnostics, diagnostics)))
          return tab;
        changed = true;
  ...
  if (!active?.validation) {
    validationBody = (
      <div className="scriptValidationSummary">
        <span>正在校验当前草稿…</span>
      </div>
    );
  ...
  const format = useCallback((tab: ScriptTab) => {
    try {
      ...
      setTabs((current) =>
        current.map((item) =>
          item.id === tab.id ? { ...item, text, dirty: text !== item.base, validation: undefined } : item,
        ),
      );
  ...
  const renamePlugin = useCallback(
    async (name: string) => {
      ...
      try {
        ...
        setTabs((current) => current.map((tab) => (tab.id === oldId ? { ...tab, id: nextId, path: [nextName] } : tab)));
    ...
    <div className="scriptsWorkspace" data-test-id="scripts-workspace">
      <aside className="scriptNavigation">
        <div className="scriptRootTabs">
          <button
            className={activeScriptWorkspace === 'functions' ? 'is-active' : ''}
            onClick={() => setActiveScriptWorkspace('functions')}
          >
            函数
          </button>
          <button
            className={activeScriptWorkspace === 'plugins' ? 'is-active' : ''}
            onClick={() => setActiveScriptWorkspace('plugins')}
          >
            插件
          </button>
          {activeScriptWorkspace === 'plugins' ? (
            <button aria-label="新增插件" onClick={() => void addPlugin()}>
              <Plus size={15} />
            </button>
          ) : null}
        </div>
      ...
      <main className="scriptEditorWorkspace">
        <div className="scriptTabs">
          {tabs.map((tab) => (
            <button
              className={tab.id === activeId ? 'scriptTab is-active' : 'scriptTab'}
              key={tab.id}
              onClick={() => setActiveId(tab.id)}
            >
              <span>{tab.path.at(-1)}</span>
              {tab.dirty ? (
                <i title="未保存">
                  <Circle fill="currentColor" size={12} strokeWidth={0} />
                </i>
              ) : null}
              {tab.conflict || tab.diskDeleted ? <b title="磁盘版本已变化">!</b> : null}
              <span
                role="button"
                aria-label="关闭"
                onClick={(event) => {
        ...
        {active ? (
          <>
            <div className="scriptEditorToolbar">
              <span>
                {active.kind === 'functions' ? '函数' : '插件'} / {active.path.join(' / ')}
              </span>
                ...
                <div className="scriptConflictActions">
                  ...
                  <button
                    onClick={() =>
                      setTabs((current) =>
                        current.map((tab) =>
                          tab.id === active.id ? { ...tab, conflict: undefined, diskDeleted: false } : tab,
                        ),
                      )
                    }
                  >
              ...
              <button data-test-id="script-format" title="格式化当前草稿，不会自动保存" onClick={() => format(active)}>
              ...
              <button disabled={!active.dirty} onClick={() => void save(active)}>
                保存
              </button>
              {customPlugin ? (
                <div className="scriptPluginActions">
                  <button onClick={() => void renamePlugin(customPlugin)}>重命名</button>
                  <button className="danger" onClick={() => deletePlugin(customPlugin)}>
                    删除
                  </button>
                </div>
              ) : null}
