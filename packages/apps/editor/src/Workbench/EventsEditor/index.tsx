...
import { loadProjectBlockPack, subscribeProjectBlockPack } from '@/blockly/project';
import { CustomBlockManager, type CustomBlockRegistrationSeed } from './CustomBlockManager';
...
export const EventsEditor: FC = () => {
  ...
  // 可选链静默丢弃。
  useEffect(() => {
    const api = createEditorBlocklyApi(() => workspaceRef.current, showEditor, hideEditor, {
    ...
    const openRequest = (request: EventEditorOpenRequest) => {
      ...
      const source =
        isCommonEventWorkspace && typeof request.initialValue === 'string'
    ...
    const unregister = registerEventEditor((request) => {
      ...
      void loadProjectBlockPack()
        .catch((error) =>
          reportInteraction(`自定义事件块加载失败：${error instanceof Error ? error.message : String(error)}`, 'error'),
        )
    ...
    >
      <div style={{ position: 'relative', height: '95%' }}>
        ...
        <h3 className="eventEditorToolbar">
          事件编辑器 (V12) &nbsp;&nbsp;
          {embedded ? (
            <button data-test-id="event-editor-save" onClick={handleApply}>
              保存
            </button>
          ) : (
            <>
              <button data-test-id="event-editor-confirm" onClick={handleConfirm}>
                确认
              </button>
              <button data-test-id="event-editor-apply" onClick={handleApply}>
                应用
              </button>
            </>
          )}
          ...
          {embedded ? null : (
            <button data-test-id="event-editor-cancel" onClick={handleCancel}>
              取消
            </button>
          )}
          <div
          ...
          <input
            ...
            style={{ marginLeft: 10 }}
          />
          <span className="cpPanel" style={{ marginLeft: '-4px', fontSize: 13 }}>
            开启中文名替换
          </span>
          <input
            type="checkbox"
  ...
  return embeddedHost ? (
    <>
      {createPortal(editor, embeddedHost)}
      {manager}
    </>
  ) : (
    <>
