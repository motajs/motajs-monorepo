...
export const SelectPointContent: FC<SelectPointContentProps> = ({
  ...
  const fallbackFloorId = floorOptions.includes(storeFloorId ?? "")
    ? storeFloorId!
    : floorOptions[0];
  const fixedFloorId = initialFloorId && floorOptions.includes(initialFloorId)
    ? initialFloorId
    : fallbackFloorId;
  ...
  const setPoint = useCallback((targetFloorId?: string, nextX?: number, nextY?: number) => {
    const nextFloorId = floorSelection === "fixed"
      ? fixedFloorId
  ...
  const handleCanvasDoubleClick = useCallback((gridPos: LocPOD) => {
