...
export const ModalsProvider: FC<ModalsProviderProps> = ({ children }) => {
  ...
  useEffect(() => registerCodeEditorPreview(async (mode, value) => {
  ...
  const blocklyCapabilities = useMemo(() => ({
    editText: (request: { contextId: string; value: string; lint?: boolean }) => new Promise<string | null>((resolve) => {
      codeEditor.open({
    ...
    selectMaterial: (request: {
      ...
      const nameMap = tower.status === "loaded"
        ? (tower.value.main.nameMap ?? {}) as Record<string, string>
        : {};
      ...
      return openSelectMaterial({
        ...
        source: request.kind === "image" || request.kind === "hero"
        ...
        transform: request.transform === "strip-animate-extension"
          ? (name: string) => name.replace(/\.animate$/i, "")
          : null,
      }).then((result) => {
    ...
    preview: async (result: import("@/blockly/interactions/previewModel").BlocklyPreviewResult) => {
      ...
      const runtimeList = list.length > 0
        ? list
    ...
    searchFlags: async () => { await openSearchFlags({}); },
      ...
      <SelectPointModalActionProvider open={openSelectPoint}>
        <SelectMaterialModalActionProvider open={openSelectMaterial}>
          <CheckboxSetModalActionProvider open={openCheckboxSet}>
            <BlocklyCapabilitiesProvider value={blocklyCapabilities}>
              {children}
            </BlocklyCapabilitiesProvider>
