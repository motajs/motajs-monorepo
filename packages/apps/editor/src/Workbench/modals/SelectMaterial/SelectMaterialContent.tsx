...
const CroppedImagePreview: FC<{ path: string; crop: NonNullable<ProjectImageEntry["crop"]> }> = ({ path, crop }) => {
  ...
  return <canvas data-test-id="select-material-crop-preview" ref={canvasRef} style={{ display: "block", maxWidth: "100%" }} />;
};
...
export const SelectMaterialContent: FC<SelectMaterialContentProps> = ({
  ...
  const resolvedSource = useMemo<SelectMaterialOptions["source"]>(() => source ?? (
    ...
    if (resolvedSource?.kind === "project-images") {
      ...
      const dispose = resource.subscribe((content) => {
        if (content.status === "loaded") {
          setProjectImages(content.value.entries.filter((entry) => resolvedSource.includeLogical || entry.kind === "physical"));
        }
  ...
  return (
    <div data-test-id="select-material-content" style={{ display: "block", marginTop: -10 }}>
      <div style={{ marginLeft: 10, lineHeight: "25px" }}>
        {multiple && (
          <>
            <button data-test-id="select-material-all" onClick={() => onChange(entries.map((entry) => entry.name))}>全选</button>
            <button data-test-id="select-material-none" style={{ marginLeft: 10 }} onClick={() => onChange([])}>全不选</button>
            <br />
          </>
        )}
        {entries.map((entry) => (
          <div key={`${entry.path}:${entry.name}`} style={{ display: "block" }}>
            ...
            {preview[entry.name] && entry.isImage && (entry.crop
