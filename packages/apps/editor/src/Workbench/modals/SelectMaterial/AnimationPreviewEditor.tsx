...
const CueSoundPreview: FC<{ sound: string; pitch: number }> = ({ sound, pitch }) => {
  ...
  return (
    <button data-test-id="animation-cue-preview" onClick={play}>
      试听
    </button>
  );
};
      ...
      <div data-test-id="animation-cues" style={{ marginTop: 8 }}>
        {cues.map((cue, index) => (
          <div key={`${index}-${cue.frame}`} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
            ...
            >
              {Array.from({ length: frameMax }, (_, frame) => (
                <option key={frame + 1}>{frame + 1}</option>
              ))}
            </select>
            <span>帧：</span>
            <input
            ...
            />
            <button
              data-test-id={`animation-cue-delete-${index}`}
              onClick={() => setCues((current) => current.filter((_, cueIndex) => cueIndex !== index))}
            >
              删除
            </button>
          </div>
        ))}
        <datalist id="animation-sound-options">
          {sounds.map((sound) => (
        ...
        <button data-test-id="animation-cue-save" style={{ marginLeft: 10 }} onClick={() => void save()}>
          保存
        </button>
      </div>
    </div>
