      ...
      <p id="_previewStatusBarP" style={{ display: 'flex', flexWrap: orientation === 'vertical' ? 'wrap' : 'nowrap' }}>
        <span>
          ...
          {!lease && (runtime.state.status === 'error' || runtimeError) ? (
            <span data-test-id="runtime-status-bar-error" style={{ display: 'block', width: 240 }}>
              Runtime 不可用，无法预览状态栏。
              <span data-test-id="runtime-status-bar-error-message">
                {runtimeError?.message ?? runtime.state.error?.message}
              </span>
              <button data-test-id="runtime-retry" onClick={() => void runtime.retry()}>
                重试
              </button>
            </span>
          ) : null}
        ...
        <span style={{ margin: 10 }} id="_previewStatusBarValue">
          ...
          <input
            style={{ width: 50 }}
            value={values.hpmax}
            onChange={(event) => setValues((prev) => ({ ...prev, hpmax: event.target.value }))}
          />
          攻击:
          <input
            style={{ width: 50 }}
            value={values.atk}
            onChange={(event) => setValues((prev) => ({ ...prev, atk: event.target.value }))}
          />
          防御:
          <input
            style={{ width: 50 }}
            value={values.def}
            onChange={(event) => setValues((prev) => ({ ...prev, def: event.target.value }))}
          />
          护盾:
          <input
            style={{ width: 50 }}
            value={values.mdef}
            onChange={(event) => setValues((prev) => ({ ...prev, mdef: event.target.value }))}
          />
          魔力:
          <input
            style={{ width: 50 }}
            value={values.mana}
            onChange={(event) => setValues((prev) => ({ ...prev, mana: event.target.value }))}
          />
          上限:
          <input
            style={{ width: 50 }}
            value={values.manamax}
            onChange={(event) => setValues((prev) => ({ ...prev, manamax: event.target.value }))}
          />
          金币:
          <input
            style={{ width: 50 }}
            value={values.money}
            onChange={(event) => setValues((prev) => ({ ...prev, money: event.target.value }))}
          />
          经验:
          <input
            style={{ width: 50 }}
            value={values.exp}
            onChange={(event) => setValues((prev) => ({ ...prev, exp: event.target.value }))}
          />
          等级:
          <input
            style={{ width: 50 }}
            value={values.lv}
            onChange={(event) => setValues((prev) => ({ ...prev, lv: event.target.value }))}
          />
          <br />
          当前道具ID（以逗号分隔）：
          <br />
