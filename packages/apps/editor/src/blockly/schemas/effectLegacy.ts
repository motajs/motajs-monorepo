...
export const loadBgmSchema = declarativeSchema(
  'loadBgm',
  'effect',
  {
    type: 'mota_loadBgm_s',
    message0: '预加载背景音乐 %1',
    args0: [{ type: 'field_input', name: 'NAME', text: 'bgm.mp3' }],
    colour: 20,
  },
...
export const freeBgmSchema = declarativeSchema(
  'freeBgm',
  'effect',
  {
    type: 'mota_freeBgm_s',
...
export const setBgmSpeedSchema = declarativeSchema(
  'setBgmSpeed',
  'effect',
  {
    type: 'mota_setBgmSpeed_s',
...
export const showTextImageSchema: BlockSchema = {
  eventType: 'showTextImage',
  category: 'effect',
  definition: {
    ...statementDefinition,
    type: 'mota_showTextImage_s',
    colour: 20,
    inputsInline: false,
    message0: '显示图片化文本 %1',
  ...
  parser: (event) => {
    ...
    return {
      type: 'mota_showTextImage_s',
      fields: {
        TEXT: expression(event.text),
        CODE: event.code ?? 1,
        X: loc[0],
        Y: loc[1],
        LINE_HEIGHT: event.lineHeight ?? 1.4,
        REVERSE: event.reverse ?? 'null',
        OPACITY: expression(event.opacity),
  ...
  generator: (block) => {
    const event: Record<string, unknown> = {
      ...
      code: Number(block.getFieldValue('CODE')),
    };
    writePair(event, 'loc', block.getFieldValue('X'), block.getFieldValue('Y'));
    const lineHeight = Number(block.getFieldValue('LINE_HEIGHT'));
    if (lineHeight !== 1.4) event.lineHeight = lineHeight;
...
function transformImageSchema(eventType: 'rotateImage' | 'scaleImage'): BlockSchema {
  ...
  return {
    ...
    parser: (event) => {
      ...
      return {
        ...
        fields: {
          ...
          X: center[0],
          Y: center[1],
          MOVE_MODE: event.moveMode ?? '',
          AMOUNT: event[rotate ? 'angle' : 'scale'] ?? (rotate ? 90 : 0.8),
          TIME: event.time ?? (rotate ? 500 : 0),
          ASYNC: event.async === true,
        },
      };
    },
    generator: (block) => {
      ...
      const mode = block.getFieldValue('MOVE_MODE');
      if (mode) event.moveMode = mode;
      if (checkbox(block, 'ASYNC')) event.async = true;
      return generated(event);
    },
  };
