...
function buildDoorKeyBlocks(keys: RecordValue): BlockState | undefined {
  return chain(Object.entries(keys).sort(([left], [right]) => left.localeCompare(right)).map(([rawKey, rawAmount]) => {
...
export const doorInfoEntrySchema: BlockSchema = {
  ...
  definition: {
    ...
    colour: 'auto', tooltip: '开门信息', helpUrl: '/_docs/#/instruction',
  },
  category: 'entry',
  parser: (value: unknown, context) => {
    ...
    return {
      ...
      inputs: {
        ...
        AFTER_OPEN_DOOR: statementInput(parseEventList(
          Array.isArray(info.afterOpenDoor) ? info.afterOpenDoor as EventData[] : [], context,
        ) ?? undefined),
      },
    };
  },
...
export const levelChooseItemSchema = itemSchema({
  type: 'mota_levelChooseItem', eventType: '_levelChooseItem', connection: 'levelChooseItem',
  ...
  message1: '执行 %1', args1: [{ type: 'input_statement', name: 'ACTION' }],
...
export const levelChooseEntrySchema: BlockSchema = {
  ...
  definition: {
    type: 'mota_levelChoose_m', message0: '难度分歧 %1',
    args0: [{ type: 'input_statement', name: 'ITEMS', check: 'levelChooseItem' }],
    colour: 'auto', tooltip: '难度分歧', helpUrl: '/_docs/#/instruction',
  }, category: 'entry',
  parser: (value: unknown, context) => ({
    type: 'mota_levelChoose_m',
    inputs: { ITEMS: statementInput(chain((Array.isArray(value) ? value : []).map((raw): BlockState => {
      const item = record(raw);
      return {
        type: 'mota_levelChooseItem',
        fields: { TITLE: item.title ?? '', NAME: item.name ?? '', HARD: item.hard ?? 0, COLOR: parseColour(item.color) },
        inputs: { ACTION: statementInput(parseEventList(Array.isArray(item.action) ? item.action as EventData[] : [], context) ?? undefined) },
      };
    }))) },
  }),
...
export const floorPartitionItemSchema = itemSchema({
  type: 'mota_floorPartitionItem', eventType: '_floorPartitionItem', connection: 'floorPartitionItem',
  message0: '分区项 起始楼层ID %1 终止楼层ID（不填代表到最后一层） %2',
  args0: [
    { type: 'field_input', name: 'START', text: 'MTx' },
    { type: 'field_input', name: 'END', text: '' },
  ],
  generator: (block) => `${JSON.stringify([
...
export const floorPartitionEntrySchema: BlockSchema = {
  ...
  definition: {
    type: 'mota_floorPartition_m', message0: '高层塔分区管理 %1',
    args0: [{ type: 'input_statement', name: 'ITEMS', check: 'floorPartitionItem' }],
    colour: 'auto', tooltip: '高层塔分区管理', helpUrl: '/_docs/#/instruction',
  }, category: 'entry',
  parser: (value: unknown) => ({
    type: 'mota_floorPartition_m',
    inputs: { ITEMS: statementInput(chain((Array.isArray(value) ? value : []).map((item) => ({
      type: 'mota_floorPartitionItem', fields: { START: Array.isArray(item) ? item[0] ?? '' : '', END: Array.isArray(item) ? item[1] ?? '' : '' },
    })))) },
  }),
  generator: (block) => `[\n${javascriptGenerator.statementToCode(block, 'ITEMS')}]\n`,
};
...
function equipItemSchema(known: boolean): BlockSchema {
  return itemSchema({
    ...
    eventType: known ? '_equipKnown' : '_equipUnknown', connection: 'equipItem',
    message0: '%1 : %2',
    args0: [
      known
    ...
    generator: (block) => `${JSON.stringify(String(block.getFieldValue('KEY') ?? ''))}: ${JSON.stringify(scalar(block.getFieldValue('VALUE')))},\n`,
  });
}
...
function equipBlocks(value: unknown): BlockState | undefined {
  return chain(Object.entries(record(value)).sort(([a], [b]) => a.localeCompare(b)).map(([key, item]) => ({
...
export const equipEntrySchema: BlockSchema = {
  ...
  definition: {
    type: 'mota_equip_m', message0: '装备 类型 %1 装备动画（第一个装备格有效） %2',
    args0: [
      { type: 'field_input', name: 'TYPE', text: '0' },
      { type: 'field_input', name: 'ANIMATE', text: '' },
    ],
    message1: '数值提升项 %1', args1: [{ type: 'input_statement', name: 'VALUE', check: 'equipItem' }],
    message2: '百分比提升项 %1', args2: [{ type: 'input_statement', name: 'PERCENTAGE', check: 'equipItem' }],
    message3: '穿上时事件 %1', args3: [{ type: 'input_statement', name: 'EQUIP_EVENT' }],
    message4: '脱下时事件 %1', args4: [{ type: 'input_statement', name: 'UNEQUIP_EVENT' }],
    message5: '此道具cls须为equips并设置canUseItemEffect',
    colour: 'auto', tooltip: '装备', helpUrl: '/_docs/#/instruction',
  }, category: 'entry',
  interactions: [{ type: 'selectMaterial', field: 'ANIMATE', materialKind: 'animate', transform: 'strip-animate-extension' }],
  parser: (value: unknown, context) => {
    ...
    return {
      ...
      inputs: {
        ...
        EQUIP_EVENT: statementInput(parseEventList(Array.isArray(item.equipEvent) ? item.equipEvent as EventData[] : [], context) ?? undefined),
        UNEQUIP_EVENT: statementInput(parseEventList(Array.isArray(item.unequipEvent) ? item.unequipEvent as EventData[] : [], context) ?? undefined),
      },
    };
  },
  generator: (block) => {
    ...
    return JSON.stringify({
      ...
      value: parseMap('VALUE'), percentage: parseMap('PERCENTAGE'),
...
export const floorImageItemSchema = itemSchema({
  type: 'mota_floorImageItem', eventType: '_floorImageItem', connection: 'floorImageItem',
  message0: '图片名 %1 翻转 %2 图层 %3 绘制坐标 x %4 y %5 初始禁用 %6',
  args0: [
    { type: 'field_input', name: 'NAME', text: 'bg.jpg' },
    { type: 'field_dropdown', name: 'REVERSE', options: [['不翻转', ''], ['水平', ':x'], ['垂直', ':y'], ['中心', ':o']] },
    { type: 'field_dropdown', name: 'CANVAS', options: [['背景层', 'bg'], ['前景层', 'fg']] },
    { type: 'field_number', name: 'X', value: 0 }, { type: 'field_number', name: 'Y', value: 0 },
  ...
  generator: (block) => {
    ...
    return `${JSON.stringify({
      ...
      x: Number(block.getFieldValue('X')) || 0, y: Number(block.getFieldValue('Y')) || 0,
      ...(block.getFieldValue('DISABLE') === 'TRUE' ? { disable: true } : {}),
      ...Object.fromEntries(['SX', 'SY', 'W', 'H', 'FRAME'].flatMap((name) => {
...
export const floorImageEntrySchema: BlockSchema = {
  ...
  definition: {
    type: 'mota_floorImage_m', message0: '楼层贴图 %1',
    args0: [{ type: 'input_statement', name: 'ITEMS', check: 'floorImageItem' }],
    colour: 'auto', tooltip: '楼层贴图', helpUrl: '/_docs/#/instruction',
  }, category: 'entry',
  parser: (value: unknown) => ({
    type: 'mota_floorImage_m',
    inputs: { ITEMS: statementInput(chain((Array.isArray(value) ? value : []).map((raw) => {
      const item = record(raw);
      return { type: 'mota_floorImageItem', fields: {
        NAME: item.name ?? '', REVERSE: item.reverse ?? '', CANVAS: item.canvas ?? 'bg',
        X: item.x ?? 0, Y: item.y ?? 0, DISABLE: item.disable === true,
        SX: item.sx ?? '', SY: item.sy ?? '', W: item.w ?? '', H: item.h ?? '', FRAME: item.frame ?? '',
      } };
    }))) },
  }),
  generator: (block) => `[\n${javascriptGenerator.statementToCode(block, 'ITEMS')}]\n`,
...
export const faceIdsEntrySchema: BlockSchema = {
  ...
  definition: {
    type: 'mota_faceIds_m', message0: '行走图朝向',
    message1: '向下ID %1 向左ID %2 向右ID %3 向上ID %4',
    args1: ['DOWN', 'LEFT', 'RIGHT', 'UP'].map((name) => ({ type: 'field_input', name, text: '' })),
    colour: 'auto', tooltip: '行走图朝向', helpUrl: '/_docs/#/instruction',
  }, category: 'entry',
  parser: (value: unknown) => {
    const item = record(value);
    return { type: 'mota_faceIds_m', fields: { DOWN: item.down ?? '', LEFT: item.left ?? '', RIGHT: item.right ?? '', UP: item.up ?? '' } };
  },
  generator: (block) => JSON.stringify(Object.fromEntries([
    ['down', block.getFieldValue('DOWN')], ['left', block.getFieldValue('LEFT')],
    ['right', block.getFieldValue('RIGHT')], ['up', block.getFieldValue('UP')],
  ].filter(([, value]) => String(value ?? '') !== ''))),
};

export const splitImageItemSchema = itemSchema({
  type: 'mota_splitImageItem', eventType: '_splitImageItem', connection: 'splitImageItem',
  ...
  generator: (block) => `${JSON.stringify({
    name: String(block.getFieldValue('NAME') ?? ''), width: Number(block.getFieldValue('WIDTH')) || 0,
    height: Number(block.getFieldValue('HEIGHT')) || 0, prefix: String(block.getFieldValue('PREFIX') ?? ''),
  })},\n`,
});
...
export const splitImagesEntrySchema: BlockSchema = {
  ...
  definition: {
    type: 'mota_splitImages_m', message0: '图片切分（你可以将一张png格式的大图切分为若干小图） %1',
    args0: [{ type: 'input_statement', name: 'ITEMS', check: 'splitImageItem' }],
    colour: 'auto', tooltip: '图片裁剪', helpUrl: '/_docs/#/instruction',
  }, category: 'entry',
  parser: (value: unknown) => ({
    type: 'mota_splitImages_m',
    inputs: { ITEMS: statementInput(chain((Array.isArray(value) ? value : []).map((raw) => {
      const item = record(raw);
      return { type: 'mota_splitImageItem', fields: { NAME: item.name ?? '', WIDTH: item.width ?? 32, HEIGHT: item.height ?? 32, PREFIX: item.prefix ?? '' } };
    }))) },
  }),
  generator: (block) => `[\n${javascriptGenerator.statementToCode(block, 'ITEMS')}]\n`,
};
...
const STYLE_FIELDS: Array<[string, string]> = [
  ['START_BACKGROUND', 'startBackground'], ['START_VERTICAL_BACKGROUND', 'startVerticalBackground'],
  ['START_LOGO_STYLE', 'startLogoStyle'], ['START_BUTTONS_STYLE', 'startButtonsStyle'],
  ['STATUS_LEFT_BACKGROUND', 'statusLeftBackground'], ['STATUS_TOP_BACKGROUND', 'statusTopBackground'],
  ['TOOLS_BACKGROUND', 'toolsBackground'], ['FLOOR_CHANGING_STYLE', 'floorChangingStyle'],
  ['FONT', 'font'],
];
const STYLE_COLOURS: Array<[string, string]> = [
  ['STATUS_BAR_COLOR', 'statusBarColor'], ['BORDER_COLOR', 'borderColor'], ['SELECT_COLOR', 'selectColor'],
];

export const mainStyleEntrySchema: BlockSchema = {
  eventType: 'entry:mainStyle',
  definition: {
    type: 'mota_mainStyle_m', message0: '主要样式设置',
    message1: '标题界面背景图 %1', args1: [{ type: 'field_input', name: 'START_BACKGROUND', text: '' }],
    message2: '竖屏标题界面背景图 %1', args2: [{ type: 'field_input', name: 'START_VERTICAL_BACKGROUND', text: '' }],
    message3: '标题样式；可写 display: none 隐藏标题 %1', args3: [{ type: 'field_input', name: 'START_LOGO_STYLE', text: '' }],
    message4: '标题按钮样式 %1', args4: [{ type: 'field_input', name: 'START_BUTTONS_STYLE', text: '' }],
    message5: '横屏状态栏背景 %1', args5: [{ type: 'field_input', name: 'STATUS_LEFT_BACKGROUND', text: '' }],
    message6: '竖屏状态栏背景 %1', args6: [{ type: 'field_input', name: 'STATUS_TOP_BACKGROUND', text: '' }],
    message7: '竖屏工具栏背景 %1', args7: [{ type: 'field_input', name: 'TOOLS_BACKGROUND', text: '' }],
    message8: '楼层切换样式 %1', args8: [{ type: 'field_input', name: 'FLOOR_CHANGING_STYLE', text: '' }],
    message9: '全局字体 %1', args9: [{ type: 'field_input', name: 'FONT', text: '' }],
    ...
    colour: 'auto', tooltip: '主要样式设置', helpUrl: '/_docs/#/instruction',
  }, category: 'entry',
  interactions: STYLE_COLOURS.map(([field]) => ({
    type: 'colourBinding' as const, textField: field, colourField: `${field}_PICKER`,
  })),
  parser: (value: unknown) => {
    const item = record(value);
    return { type: 'mota_mainStyle_m', fields: Object.fromEntries([
...
const NAME_MAP_TYPES: Record<NameMapKind, string> = {
  systemSound: 'mota_nameMapSystemSound', sound: 'mota_nameMapSound', bgm: 'mota_nameMapBgm',
  image: 'mota_nameMapImage', animate: 'mota_nameMapAnimate', unknown: 'mota_nameMapUnknown',
};
...
function nameMapItemSchema(kind: NameMapKind, label: string, materialKind?: MaterialKind): BlockSchema {
  ...
  return itemSchema({
    type: NAME_MAP_TYPES[kind], eventType: `_nameMap:${kind}`, connection: 'nameMapItem',
    ...
    interactions: materialKind ? [{
      type: 'selectMaterial', field: 'VALUE', materialKind,
      ...(kind === 'animate' ? { transform: 'strip-animate-extension' as const } : {}),
      aliasPolicy: 'physical-name',
    }] : undefined,
    generator: (block) => `${JSON.stringify(String(block.getFieldValue('KEY') ?? ''))}: ${JSON.stringify(String(block.getFieldValue('VALUE') ?? ''))},\n`,
  });
}
...
export const nameMapEntrySchema: BlockSchema = {
  ...
  definition: {
    type: 'mota_nameMap_m', message0: '文件别名设置（可以游戏中使用此别名代替原始文件名） %1',
    args0: [{ type: 'input_statement', name: 'ITEMS', check: 'nameMapItem' }],
    colour: 'auto', tooltip: '文件别名设置', helpUrl: '/_docs/#/instruction',
  }, category: 'entry',
  parser: (value: unknown, context) => ({
    type: 'mota_nameMap_m',
    inputs: { ITEMS: statementInput(chain(Object.entries(record(value)).map(([key, rawValue]) => {
...
export const projectEntrySchemas: BlockSchema[] = [
  rawEntrySchema,
  doorInfoEntrySchema, doorKeyKnownSchema, doorKeyUnknownSchema,
  levelChooseEntrySchema, levelChooseItemSchema,
  floorPartitionEntrySchema, floorPartitionItemSchema,
  equipEntrySchema, equipKnownSchema, equipUnknownSchema,
  floorImageEntrySchema, floorImageItemSchema,
  faceIdsEntrySchema,
  splitImagesEntrySchema, splitImageItemSchema,
  mainStyleEntrySchema,
  nameMapEntrySchema, nameMapSystemSoundSchema, nameMapSoundSchema, nameMapBgmSchema,
  nameMapImageSchema, nameMapAnimateSchema, nameMapUnknownSchema,
];
