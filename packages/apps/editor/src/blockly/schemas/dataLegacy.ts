...
export const followSchema = declarativeSchema(
  'follow',
  'data',
  {
    type: 'mota_follow_s',
...
export const unfollowSchema = declarativeSchema(
  'unfollow',
  'data',
  {
    type: 'mota_unfollow_s',
    message0: '取消跟随 行走图 %1',
    args0: [{ type: 'field_input', name: 'NAME', text: '' }],
    colour: 'auto',
  },
...
export const loadEquipSchema = declarativeSchema(
  'loadEquip',
  'data',
  {
    type: 'mota_loadEquip_s',
    message0: '装上装备 %1',
    args0: [{ type: 'field_input', name: 'ID', text: 'sword1' }],
    colour: 'auto',
  },
...
export const unloadEquipSchema = declarativeSchema(
  'unloadEquip',
  'data',
  {
    type: 'mota_unloadEquip_s',
    message0: '卸下第 %1 格装备',
    args0: [{ type: 'field_number', name: 'POS', value: 0, min: 0, precision: 1 }],
    colour: 'auto',
  },
  [field('POS', 'pos', { valueType: 'number', optional: false })],
);

export const setEquipSchema = declarativeSchema(
  'setEquip',
  'data',
  {
    type: 'mota_setEquip_s',
    message0: '设置装备属性 装备 ID %1 %2 属性 %3 %4 %5',
    args0: [
...
export const resetEnemyOnPointSchema: BlockSchema = {
  eventType: 'resetEnemyOnPoint',
  category: 'data',
  definition: {
    ...statementDefinition,
    type: 'mota_resetEnemyOnPoint_s',
    colour: 'auto',
  ...
  parser: (event) =>
    locationState('mota_resetEnemyOnPoint_s', event, {
      FLOOR_ID: expression(event.floorId),
      NO_REFRESH: event.norefresh === true,
    }),
  generator: (block) => {
...
const moveDefinition = (type: string, relative: boolean): BlockSchema['definition'] => ({
  ...
  args0: [
    ...
    { type: 'field_input', name: 'TARGET_X', text: '' },
    { type: 'field_input', name: 'TARGET_Y', text: '' },
    { type: 'field_input', name: 'FLOOR_ID', text: '' },
    { type: 'field_checkbox', name: 'NO_REFRESH', checked: false },
  ],
});
...
export const moveEnemyOnPointSchema: BlockSchema = {
  eventType: 'moveEnemyOnPoint',
  category: 'data',
  definition: moveDefinition('mota_moveEnemyOnPoint_s', false),
  parser: (event: EventObject) => {
    const relative = Array.isArray(event.dxy);
    const from = pair(event.from);
    const target = pair(relative ? event.dxy : event.to);
    return {
      type: relative ? 'mota_moveEnemyOnPoint_1_s' : 'mota_moveEnemyOnPoint_s',
      fields: {
        FROM_X: from[0],
        FROM_Y: from[1],
        TARGET_X: target[0],
...
export const moveEnemyOnPointRelativeSchema: BlockSchema = {
  eventType: '_moveEnemyOnPointRelative',
  category: 'data',
  definition: moveDefinition('mota_moveEnemyOnPoint_1_s', true),
  parser: () => ({ type: 'mota_moveEnemyOnPoint_1_s' }),
  generator: moveGenerator(true),
  interactions: [{ type: 'autocomplete', field: 'FLOOR_ID', source: 'floor' }],
};
...
export const legacyDataSchemas: BlockSchema[] = [
  ...
  unloadEquipSchema,
  setEquipSchema,
  resetEnemyOnPointSchema,
  moveEnemyOnPointSchema,
  moveEnemyOnPointRelativeSchema,
];
