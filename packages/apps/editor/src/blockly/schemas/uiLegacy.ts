...
const expr = (input: string, path: string, required = false) =>
  field(input, path, {
...
export const setFilterSchema = uiSchema(
  'setFilter',
...
export const fillTextSchema = uiSchema(
  'fillText',
...
export const drawTextContentSchema = uiSchema(
  ...
  [
    ...
    { type: 'field_input', name: 'FONT_SIZE', text: '' },
  ...
  [
    ...
    expr('FONT_SIZE', 'fontSize'),
    expr('LINE_HEIGHT', 'lineHeight'),
    field('BOLD', 'bold', { valueType: 'boolean', default: false, omitWhenDefault: true }),
  ],
  {
    interactions: [
...
function lineSchema(eventType: 'drawLine' | 'drawArrow'): BlockSchema {
  return uiSchema(
    ...
    [
      ...
      { type: 'field_input', name: 'X2', text: '0' },
      { type: 'field_input', name: 'Y2', text: '0' },
      { type: 'field_input', name: 'STYLE', text: '' },
      { type: 'field_colour', name: 'STYLE_COLOUR', colour: '#ffffff' },
...
function polygonGenerator(eventType: 'fillPolygon' | 'strokePolygon') {
  return (block: Blockly.Block): string => {
    const xs = String(block.getFieldValue('XS')).split(',');
    const ys = String(block.getFieldValue('YS')).split(',');
    if (xs.length !== ys.length || (xs.length === 1 && !xs[0] && !ys[0]))
      throw new Error('多边形 x 和 y 顶点数量必须一致且不能为空');
    const event: Record<string, unknown> = {
      type: eventType,
      nodes: xs.map((x, index) => [expressionValue(x), expressionValue(ys[index])]),
    };
    const style = block.getFieldValue('STYLE');
    if (style) event.style = style.split(',').map(expressionValue);
...
function polygonSchema(eventType: 'fillPolygon' | 'strokePolygon'): BlockSchema {
  ...
  return {
    ...
    definition: {
      ...
      args0: [
        { type: 'field_input', name: 'XS', text: '0,0,100' },
        { type: 'field_input', name: 'YS', text: '0,100,0' },
        { type: 'field_input', name: 'STYLE', text: '' },
        { type: 'field_colour', name: 'STYLE_COLOUR', colour: '#ffffff' },
        ...(stroke ? [{ type: 'field_input' as const, name: 'LINE_WIDTH', text: '' }] : []),
      ],
    },
    parser: (event) => polygonState(`mota_${eventType}_s`, event, stroke),
    generator: polygonGenerator(eventType),
    interactions: [
      { type: 'preview', adapter: 'event' },
      { type: 'colourBinding', textField: 'STYLE', colourField: 'STYLE_COLOUR' },
    ],
    defaultInteraction: 'preview',
  };
}
...
function shapeSchema(eventType: 'fillEllipse' | 'strokeEllipse' | 'fillArc' | 'strokeArc'): BlockSchema {
  const arc = eventType.endsWith('Arc');
  const stroke = eventType.startsWith('stroke');
  ...
  return uiSchema(
    ...
    [
      { type: 'field_input', name: 'X', text: '0' },
      { type: 'field_input', name: 'Y', text: '0' },
      { type: 'field_input', name: 'P1', text: '100' },
      { type: 'field_input', name: 'P2', text: arc ? '0' : '100' },
    ...
    [
      expr('X', 'x', true),
      expr('Y', 'y', true),
      expr('P1', paths[0], true),
      expr('P2', paths[1], true),
      expr('P3', paths[2], arc),
      colour('STYLE', 'style'),
      ...(stroke ? [expr('LINE_WIDTH', 'lineWidth')] : []),
    ],
  );
}
...
export const drawImageSchema: BlockSchema = {
  ...
  definition: {
    ...statementDefinition,
    type: 'mota_drawImage_s',
    colour: 359,
    inputsInline: false,
    message0: '绘制图片 %1 翻转 %2',
    args0: [
    ...
    message2: '裁剪区域 x %1 y %2 宽 %3 高 %4 旋转 %5',
    args2: [
      { type: 'field_input', name: 'X1', text: '' },
      { type: 'field_input', name: 'Y1', text: '' },
      { type: 'field_input', name: 'W1', text: '' },
      { type: 'field_input', name: 'H1', text: '' },
  ...
  parser: (event) => ({
    type: 'mota_drawImage_s',
    fields: {
      IMAGE: expression(event.image),
      REVERSE: event.reverse ?? 'null',
      X: expression(event.x),
      Y: expression(event.y),
      W: expression(event.w),
      H: expression(event.h),
      X1: expression(event.x1),
      Y1: expression(event.y1),
      W1: expression(event.w1),
      H1: expression(event.h1),
  ...
  generator: (block) => {
    ...
    if (reverse !== 'null') event.reverse = reverse;
  ...
  interactions: [
    ...
    { type: 'autocomplete', field: 'IMAGE', source: 'image' },
    { type: 'preview', adapter: 'event' },
  ],
  defaultInteraction: 'preview',
};
