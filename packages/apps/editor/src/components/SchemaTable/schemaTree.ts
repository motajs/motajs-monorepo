...
export function listNodeLocations(schema: UISchema): NodeLocation[] {
  ...
  const visit = (nodes: UINode[], parentId?: string): void =>
...
export function removeNode(
  schema: UISchema,
  id: string,
): { schema: UISchema; removed: UINode; location: NodeLocation } {
  const next = cloneSchema(schema);
  const location = findNodeLocation(next, id);
  if (!location) throw new Error(`Unknown UI node: ${id}`);
...
export function restoreNodeProperties(current: UISchema, builtin: UISchema, id: string): UISchema {
  ...
  if (!currentNode || !builtinNode || currentNode.kind !== builtinNode.kind)
    throw new Error('内置 Schema 中没有同类型节点');
...
export function restoreNodePosition(current: UISchema, builtin: UISchema, id: string): UISchema {
  ...
  const preceding = builtinSiblings
    .slice(0, baseline.index)
    .map((node) => node.id)
    .reverse();
  const following = builtinSiblings.slice(baseline.index + 1).map((node) => node.id);
  const afterIndex = preceding
    .map((nodeId) => siblings.findIndex((node) => node.id === nodeId))
    .find((index) => index >= 0);
  const beforeIndex = following
    .map((nodeId) => siblings.findIndex((node) => node.id === nodeId))
    .find((index) => index >= 0);
