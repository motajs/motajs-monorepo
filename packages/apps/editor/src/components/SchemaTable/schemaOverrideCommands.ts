...
function collectFieldReferences(value: unknown, path = "nodes"): Array<{ fieldId: string; node: string }> {
  ...
  const own = input.kind === "field" && typeof input.fieldSchema === "string"
    ? [{ fieldId: input.fieldSchema, node: typeof input.id === "string" ? input.id : path }]
    : [];
  return [...own, ...Object.entries(input).flatMap(([key, child]) => (
...
async function validateProspectiveSources(
  ...
  await FileHandlerManager.loadAll(all.flatMap((current) => [
  ...
  for (const current of all) {
    try {
      const source = current === fieldDefinition && fieldValue != null
        ? fieldValue
        : await currentSource(current, "field");
      const bundle = parseFieldSchemaBundle(source);
      if (bundle.schemaId !== current.fieldBundle.schemaId) throw new Error(`schemaId 必须保持为 ${current.fieldBundle.schemaId}`);
      bundles.push(bundle);
  ...
  if (editedField && fieldValue != null) {
    const removed = new Set(Object.keys(previousEditedField?.fields ?? {}).filter((id) => !Object.prototype.hasOwnProperty.call(editedField?.fields, id)));
  ...
  if (uiValue != null || fieldValue != null) {
    ...
    if (editedUi.schemaId !== tableDefinition.uiSchema.schemaId) throw new Error(`schemaId 必须保持为 ${tableDefinition.uiSchema.schemaId}`);
  }
...
export async function loadRawSchemaOverride(
  definition: BuiltinSchemaDefinition,
  layer: SchemaLayer,
): Promise<string> {
  const path = schemaOverridePath(definition, layer);
...
export async function validateAndNormalizeSchemaOverride(
  ...
  const parsed = layer === "field"
    ? parseFieldSchemaBundle(value)
    : parseUISchema(value, definition.fieldSchemas);
...
export async function saveSchemaSources(
  ...
  if (changes.field != null && parsed.field) {
    ...
    operations.push(operationForSource(fieldDefinition, "field", await withFork(fieldDefinition, "field", parsed.field) as FieldSchemaBundle, label));
  }
  if (changes.ui != null && parsed.ui) {
    await fs.promises.mkdir(".metaphysics/schemas/ui");
    operations.push(operationForSource(definition, "ui", await withFork(definition, "ui", parsed.ui) as UISchema, label));
  }
...
export async function deleteSchemaOverride(
  definition: BuiltinSchemaDefinition,
  layer: SchemaLayer,
): Promise<void> {
  await operationHistory.execute(deleteTextFileOperation(schemaOverridePath(definition, layer), {
