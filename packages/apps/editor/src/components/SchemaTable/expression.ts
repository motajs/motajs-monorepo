...
function ready(value: unknown): BlockResolution<unknown> { return { status: "ready", value }; }
...
export function evaluateExpression(
  ...
  if ("call" in expression) {
    ...
    try { return ready(callable(...args.value)); }
    catch (error) { return { status: "error", error: error instanceof Error ? error : new Error(String(error)) }; }
  }
  ...
  switch (operator) {
    ...
    case "in":
      if (values.length !== 2 || !Array.isArray(values[1])) return mismatch(operator, "a value and an array", values, path);
