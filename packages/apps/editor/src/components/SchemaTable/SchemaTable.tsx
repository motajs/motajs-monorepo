...
import { type FC, Fragment, type HTMLAttributes, type ReactNode, Suspense, useCallback, useEffect, useMemo, useRef, useState } from "react";
...
function NodeMenuButton({ nodeId, customization }: { nodeId: string; customization?: SchemaTableCustomization }) {
  ...
  return (
    <span className="schemaNodeMenu" onClick={(event) => event.stopPropagation()}>
      <button
        type="button"
        className="schemaTableIconButton schemaNodeMenuButton"
        aria-label={`节点菜单 ${nodeId}`}
      ><MoreHorizontal size={14} /></button>
      <span className="schemaNodeMenuPopup">
        <button type="button" onClick={() => customization.onSelect(nodeId)}>编辑节点</button>
        <button type="button" className="danger" onClick={() => customization.onDelete(nodeId)}>删除节点</button>
      </span>
    </span>
  );
}

const copyableEditorKinds = new Set([
  "json",
  "event",
  "code",
  "point",
...
function FieldLabel({ schema, sourceKey }: { schema: FieldSchema; sourceKey: string }) {
  const descriptions = [
    schema.description,
    ...Object.values(schema.diagnostics ?? {}),
    ...
    <Tooltip
      ...
      title={(
        <div className="schemaTableFieldTooltip">
          ...
          {descriptions.map((line) => <div key={line}>{line}</div>)}
        </div>
      )}
    >
      {label}
...
function BlockRow({ resolution, label, nodeId, customization }: {
  ...
  return (
    <tr {...row} data-test-id={`schema-block-${label}`}>
      <td><div className="schemaNodeLabel"><span>{label}</span>{nodeId ? <NodeMenuButton nodeId={nodeId} customization={customization} /> : null}</div></td>
      <td>
...
function HiddenFieldRow({ node, label, customization }: {
  node: FieldNode;
  label: string;
  customization: SchemaTableCustomization;
}) {
  ...
  return (
    <tr {...row} className={`schemaTableInactive ${row.className ?? ""}`} data-test-id={`schema-hidden-field-${node.id}`}>
      <td><div className="schemaNodeLabel"><strong>{label}</strong><NodeMenuButton nodeId={node.id} customization={customization} /></div></td>
      <td><span className="schemaTableMuted">Condition 当前为 hidden；结构编辑模式仍保留节点。</span></td>
    </tr>
  );
}

function JsonEditor({ value, disabled, onCommit }: {
  value: unknown;
  disabled?: boolean;
  onCommit(value: unknown): void | Promise<void>;
}) {
  const serialized = JSON.stringify(value, null, 2) ?? "null";
  const [draft, setDraft] = useState(() => ({ source: serialized, text: serialized, error: undefined as string | undefined }));
...
function TextEditor({ value, disabled, numericType, onCommit }: {
  value: unknown;
  disabled?: boolean;
...
function RawFallback({ source, rawValue, error, label, disabled, diagnostics, nodeId, customization }: {
  ...
  const row = nodeId ? customizationRowAttributes(nodeId, customization) : {};
  return (
    <tr {...row} className={`schemaTableMismatch${row.className ? ` ${row.className}` : ""}`} data-test-id={`schema-raw-fallback-${label}`}>
      <td>
        <div className="schemaNodeLabel"><span>{label}</span>{nodeId ? <NodeMenuButton nodeId={nodeId} customization={customization} /> : null}</div>
...
function RawCollectionFallback({ source, rawValue, issues, label, disabled, diagnostics, nodeId, customization }: {
  source: ValueSource<unknown>;
  rawValue: unknown;
  ...
  const writable = isWritableValueSource(source) && !disabled;
  const issueByKey = new Map(issues.filter((issue) => issue.path.length > 0).map((issue) => [String(issue.path[0]), issue]));
  ...
  const row = nodeId ? customizationRowAttributes(nodeId, customization) : {};
  return (
    <tr {...row} className={`schemaTableMismatch${row.className ? ` ${row.className}` : ""}`} data-test-id={`schema-raw-collection-fallback-${label}`}>
      <td>
        <div className="schemaNodeLabel"><span>{label}</span>{nodeId ? <NodeMenuButton nodeId={nodeId} customization={customization} /> : null}</div>
        <div className="schemaTableError">集合中有 {issues.length} 个值不符合静态 shape</div>
        {diagnostics?.map((diagnostic) => <div key={`${diagnostic.code}:${diagnostic.message}`} className={`schemaTableDiagnostic ${diagnostic.severity}`}>{diagnostic.message}</div>)}
      </td>
      <td>
          ...
          {entries.map(([key, value]) => {
            ...
            return (
              <div key={key} className={issue ? "invalid" : undefined}>
                ...
                {issue ? <small>{issue.message}{issue.path.length > 1 ? ` · ${issue.path.slice(1).join(".")}` : ""}</small> : null}
              </div>
            );
          })}
        </div>
...
const ExternalFieldEditor: FC<FieldEditorProps> = ({ fieldSchemaId, schema, value, disabled, scope, onCommit }) => {
  ...
  const open = () => {
    ...
    if (descriptor.kind === "event") {
      const floorIdResult = descriptor.floorId
        ? evaluateExpression(descriptor.floorId, scope)
        : undefined;
      const positionResult = descriptor.position
        ? evaluateExpression(descriptor.position, scope)
        : undefined;
      ...
      const positionRecord = positionValue && typeof positionValue === "object" && !Array.isArray(positionValue)
  ...
  return (
    <div className="schemaTableExternalValue">
      <span>{valueSummary(value)}</span>
      <button disabled={disabled} onClick={open}>编辑</button>
    </div>
  );
};
  ...
  return (
    <>
      <div data-source-status={registry.status}>
        <div className="schemaTableExternalValue">
          <span>{valueSummary(value)}</span>
          <button disabled={blocked} onClick={open}>编辑</button>
        </div>
    ...
    return value.flatMap((item) => {
      ...
      return [{
        ...
        label: typeof record.label === "string"
          ? record.label
          : typeof record.name === "string" ? record.name : String(optionValue),
      }];
    });
  }
    ...
    <div data-source-status={dynamic ? status : "ready"}>
      ...
      >
        ...
        {options.map((option) => {
          ...
          return <option key={optionValue} value={optionValue}>{option.label ?? String(option.value)}</option>;
        })}
      </select>
...
const ReferencedSelectFieldEditor: FC<FieldEditorProps & { descriptor: SelectEditorDescriptor }> = ({
  ...
  const optionSource = useMemo(
    () => resolveReference(scope, descriptor.reference!),
    [descriptor.reference, scope],
  );
  const referenced = useSourceSnapshot(optionSource);
  const options = referenced.status === "ready" && referenced.value.present
    ? referencedChoiceOptions(referenced.value.value)
    : [];
  return (
    <SelectControl
...
function itemOptions(value: unknown): ItemOption[] {
  ...
  return Object.entries(value).flatMap(([id, raw]) => {
    ...
    const equip = item.equip && typeof item.equip === "object" && !Array.isArray(item.equip)
...
const EquipmentSlotsFieldEditor: FC<FieldEditorProps> = ({ schema, value, disabled, scope, onCommit }) => {
  ...
  const slots = slotsSnapshot.status === "ready" && slotsSnapshot.value.present && Array.isArray(slotsSnapshot.value.value)
    ? slotsSnapshot.value.value.filter((item): item is string => typeof item === "string")
    : [];
  const options = itemsSnapshot.status === "ready" && itemsSnapshot.value.present
    ? itemOptions(itemsSnapshot.value.value)
    : [];
  const values = Array.isArray(value) ? [...value] : [];
  const blocked = disabled || slotsSnapshot.status !== "ready" || itemsSnapshot.status !== "ready";
  return (
    <div className="schemaEquipmentSlots">
      {slots.length === 0 ? <span className="schemaTableMuted">请先定义装备孔</span> : slots.map((slot, index) => {
...
const ItemCountRecordFieldEditor: FC<FieldEditorProps> = ({ schema, value, disabled, scope, onCommit }) => {
  ...
  const candidates = allOptions.filter((item) => descriptor.category === "equips"
    ? item.equipment || item.cls === "equips"
    : item.cls === descriptor.category);
      ...
      {rows.map(([id, count]) => {
        ...
        return (
          <div className="schemaItemCountRow" key={id}>
            ...
            />
            <button type="button" className="schemaTableIconButton danger" aria-label={`删除 ${id}`} disabled={blocked} onClick={() => {
              const next = { ...record };
              delete next[id];
              commit(next);
...
const ReferencedCheckboxSetFieldEditor: FC<FieldEditorProps> = ({ schema, value, disabled, scope, onCommit }) => {
  ...
  const descriptor = schema.editor;
  const registrySource = useMemo(
    () => resolveReference(scope, descriptor.kind === "checkboxSet" && descriptor.reference
  ...
  const selected = Array.isArray(value)
    ...
    : [];
  const options = registry.status === "ready" && registry.value.present
    ? checkboxSetOptions(registry.value.value)
    : [];
  ...
  return (
    <div data-source-status={registry.status}>
      <div className="schemaTableExternalValue">
        ...
        <button data-test-id="schema-checkbox-set-open" disabled={blocked} onClick={open}>选择</button>
      </div>
      {registry.status === "loading" ? <div className="schemaTableSkeleton" aria-label="loading" /> : null}
      {registry.status === "error" || registry.status === "type-mismatch"
        ? <div className="schemaTableError">{registry.error.message}</div>
        ...
        if (input.editor.kind === "suggestion") {
          ...
          return (
            <AutoComplete
              ...
              filterOption={(query, option) => String(option?.value ?? "").toLowerCase().includes(query.toLowerCase())}
...
const InitialPositionFieldEditor: FC<FieldEditorProps> = ({
  schema,
  value,
  disabled,
  scope,
  ...
  const floorSource = useMemo(
    () => resolveReference(scope, descriptor.kind === "initialPosition"
      ? descriptor.floors
      : { ref: "project:invalid" }),
    [descriptor, scope],
  );
  ...
  return (
    <div className="schemaInitialPosition" data-source-status={floors.status}>
      ...
      >
        {String(record.floorId ?? "未设楼层")} · ({String(record.x ?? 0)}, {String(record.y ?? 0)})
        <span>选点</span>
      </button>
      <select
        aria-label="初始朝向"
...
const FieldEditor: FC<FieldEditorProps> = (props) => {
  ...
  if (descriptor.kind === "passability") {
    ...
    const blockValue = block?.status === "ready" && (typeof block.value === "number" || typeof block.value === "string")
      ? block.value
      : undefined;
  ...
  if (descriptor.kind === "dimensions") {
    ...
    return <span className="schemaTableDimensions">{width} × {height}</span>;
  }
...
const ReadyFieldRow: FC<ReadyFieldRowProps> = ({
  ...
  const sourceKey = parsedReferences
    ...
    .join(" + ");
  const fieldAction = (node.source && resolvedReferences[0]
    ? runtime.fieldActions?.get(formatReferencedPath(resolvedReferences[0]))
    : undefined)
    ?? runtime.fieldActions?.get(node.fieldSchema);
        ...
        <div className="schemaTableValueCell">
          ...
          {hasActions ? (
            <div className="schemaTableActions">
              {canClear ? <button disabled={disabled} onClick={() => void commit(null)}>清空</button> : null}
              {copyable
                ? (
                  <button
                    type="button"
...
function FieldRow({ node, runtime }: { node: FieldNode; runtime: RuntimeProps }) {
  ...
  const referenceIds = new Set(fieldNodeReferences(node)
    .map((reference) => formatReferencedPath(resolveBoundReference(reference, runtime.bindings))));
  const diagnostics = runtime.diagnostics?.filter((item) => referenceIds.has(item.source)) ?? [];
  if (!schema) {
    return (
      <BlockRow
  ...
  if (snapshot.status === "loading" || snapshot.status === "error") {
    return <BlockRow nodeId={node.id} customization={runtime.customization} label={schema.title} resolution={snapshot} />;
  }
  ...
  if (presentation.status !== "ready") {
    ...
    return <BlockRow nodeId={node.id} customization={runtime.customization} label={schema.title} resolution={presentation} />;
  }
  ...
  if (issues.length > 0) {
    ...
    const supportsItemRepair = issues.every((item) => item.path.length > 0)
...
function RestAdd({ baseRef, path, runtime, disabled }: {
  ...
  return (
    <div className="schemaTableRestAdd">
      ...
      />
      <button disabled={disabled} onClick={() => void add()}>新增</button>
      {error ? <div className="schemaTableError">{error}</div> : null}
    </div>
  );
}

function RestRows({ entries, baseRef, runtime, disabled }: {
  entries: RestEntry[];
  baseRef: DataReference;
  runtime: RuntimeProps;
  disabled?: boolean;
          ...
          <tr className="schemaTableRestObject">
            ...
            <td>
              <div className="schemaTableActions schemaTableRestActions">
                <button disabled={!writable} onClick={() => void remove()}>删除</button>
              </div>
            </td>
          </tr>
...
function RestSection({ node, runtime }: { node: RestNode; runtime: RuntimeProps }) {
  ...
  if (snapshot.status !== "ready") {
    const resolution = snapshot.status === "type-mismatch"
      ? { status: "error" as const, error: snapshot.error }
      : snapshot;
    return (
  ...
  const inactive = runtime.inheritedInactive || (presentation?.status === "ready"
    && (presentation.value === "inactive" || presentation.value === "hidden"));
  if (
    !snapshot.value.present || !snapshot.value.value || typeof snapshot.value.value !== "object"
  ...
  return (
    <details
      ...
      onClick={runtime.customization ? (event) => {
        event.stopPropagation();
        runtime.customization?.onSelect(node.id);
      } : undefined}
      onDragStart={runtime.customization ? (event) => {
        event.stopPropagation();
      ...
      onDrop={runtime.customization ? (event) => {
        event.preventDefault();
        event.stopPropagation();
        const source = event.dataTransfer.getData("application/x-schema-node");
    ...
    >
      <summary>
        {node.label} <span>({entries.length})</span><NodeMenuButton nodeId={node.id} customization={runtime.customization} />
      </summary>
      <table>
        <thead>
          <tr>
...
function GroupSection({ node, runtime }: {
  node: Extract<UINode, { kind: "group" }>;
  runtime: RuntimeProps;
}) {
  const boundScope = useMemo(
    () => createBoundSchemaScope(runtime.scope, node.bind),
    [node.bind, runtime.scope],
  );
  ...
  if (presentation.status === "ready" && presentation.value === "hidden" && !runtime.customization) return null;
  if (presentation.status !== "ready") {
    const resolution = presentation.status === "type-mismatch"
      ? { status: "error" as const, error: presentation.error }
      : presentation;
    return (
      <section className="schemaTableGroup">
        <table>
  ...
  const childRuntime: RuntimeProps = {
    ...
    inheritedInactive: runtime.inheritedInactive || presentation.value === "inactive" || presentation.value === "hidden",
  };
  return (
    <details
      id={node.id}
    ...
    >
      <summary
        onDragOver={runtime.customization ? (event) => {
          event.preventDefault();
          event.stopPropagation();
        } : undefined}
        onDrop={runtime.customization ? (event) => {
      ...
      >{node.label}<NodeMenuButton nodeId={node.id} customization={runtime.customization} /></summary>
      <table>
        <tbody>
