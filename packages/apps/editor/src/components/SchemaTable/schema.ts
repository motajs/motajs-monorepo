...
const JSON_TYPES = new Set<JsonSchemaType>(['null', 'boolean', 'number', 'integer', 'string', 'array', 'object']);
...
function strictObject(value: unknown, name: string, keys: readonly string[]): Record<string, unknown> {
  ...
  if (unknown.length > 0)
    throw new Error(`${name} has unknown propert${unknown.length === 1 ? 'y' : 'ies'}: ${unknown.join(', ')}`);
  return input;
}
...
function jsonTypes(value: unknown, name: string): JsonShape['type'] {
  ...
  if (
    values.length === 0 ||
    values.some((item) => typeof item !== 'string' || !JSON_TYPES.has(item as JsonSchemaType))
  ) {
...
function shape(value: unknown, name: string): JsonShape {
  ...
  if (input.required != null) {
    if (
      ...
      input.required.some((item) => typeof item !== 'string' || item.length === 0)
    ) {
...
function choiceOptions(value: unknown, name: string, allowBooleanAndNull: boolean): ChoiceOption[] {
  ...
  const options = value.map((item, index) => {
    ...
    return {
      value: optionValue as ChoiceOption['value'],
...
function combineInput(value: unknown, name: string): CombineInputDescriptor {
  ...
  if (kind === 'suggestion') {
    ...
    const suggestions = strict.suggestions.map((item, index) => {
      ...
      return {
        value: string(option.value, `${name}.editor.suggestions[${index}].value`),
        label: typeof option.label === 'string' ? option.label : undefined,
      };
    });
  ...
  if (kind === 'number') {
    ...
    if (strict.integer != null && typeof strict.integer !== 'boolean')
      throw new Error(`${name}.editor.integer must be boolean`);
...
function expression(value: unknown, name: string): Expression {
  ...
  if (Object.prototype.hasOwnProperty.call(input, 'operator')) {
    ...
    return {
      operator: operator as Extract<Expression, { operator: string }>['operator'],
      args: strict.args.map((item, index) => expression(item, `${name}.args[${index}]`)),
    };
  }
  if (Object.prototype.hasOwnProperty.call(input, 'call')) {
    ...
    return {
      call: string(strict.call, `${name}.call`),
      args: (strict.args as unknown[] | undefined)?.map((item, index) => expression(item, `${name}.args[${index}]`)),
    };
  }
...
function editor(value: unknown, name: string): EditorDescriptor {
  ...
  switch (kind) {
    ...
    case 'orderedStringList': {
      ...
      return {
        kind,
        placeholder: typeof current.placeholder === 'string' ? current.placeholder : undefined,
        createLabel: typeof current.createLabel === 'string' ? current.createLabel : undefined,
      };
    }
    case 'initialPosition': {
      ...
      return {
        kind,
        floors: reference(current.floors, `${name}.floors`),
        directions:
          current.directions == null ? undefined : choiceOptions(current.directions, `${name}.directions`, false),
      };
    }
    case 'equipmentSlots': {
      ...
      return {
        kind,
        slots: reference(current.slots, `${name}.slots`),
        items: reference(current.items, `${name}.items`),
      };
    }
    case 'itemCountRecord': {
      const current = exact(['items', 'category']);
      const category = string(current.category, `${name}.category`);
      if (!(['constants', 'tools', 'equips'] as string[]).includes(category))
        throw new Error(`${name}.category is unsupported`);
      return {
        kind,
        items: reference(current.items, `${name}.items`),
        category: category as 'constants' | 'tools' | 'equips',
      };
    }
    case 'passability': {
    ...
    case 'select': {
      ...
      if ((current.options == null) === (current.reference == null))
        throw new Error(`${name} must define exactly one of options or reference`);
      return {
        kind,
        options: current.options == null ? undefined : choiceOptions(current.options, `${name}.options`, true),
        reference: current.reference == null ? undefined : reference(current.reference, `${name}.reference`),
      };
    }
    case 'checkboxSet': {
      const current = exact(['options', 'reference']);
      ...
      return current.options != null
        ? {
            kind,
            options: choiceOptions(current.options, `${name}.options`, false) as NonNullable<
              Extract<EditorDescriptor, { kind: 'checkboxSet' }>['options']
            >,
          }
    ...
    case 'event': {
      ...
      return {
        kind,
        entryType: string(current.entryType, `${name}.entryType`),
        floorId: current.floorId == null ? undefined : reference(current.floorId, `${name}.floorId`),
        position: current.position == null ? undefined : reference(current.position, `${name}.position`),
      };
    }
    case 'autoEventList': {
    ...
    case 'code': {
      ...
      return {
        kind,
        lint: current.lint === true,
        template: typeof current.template === 'string' ? current.template : undefined,
      };
    }
    case 'point': {
    ...
    case 'combine': {
      ...
      if (!Array.isArray(current.inputs) || current.inputs.length === 0)
      ...
      if (new Set(inputs.map((item) => item.key)).size !== inputs.length)
        throw new Error(`${name}.inputs keys must be unique`);
      return { kind, inputs };
    }
    case 'bgmList': {
      ...
      return {
        kind,
        reference: reference(current.reference, `${name}.reference`),
        directory: string(current.directory, `${name}.directory`),
      };
    }
    case 'floorImages': {
      const current = exact(['floorId']);
    ...
    case 'material': {
      ...
      if (current.selection !== 'single' && current.selection !== 'multiple')
        throw new Error(`${name}.selection must be single or multiple`);
      return {
        kind,
        reference: reference(current.reference, `${name}.reference`),
        directory: string(current.directory, `${name}.directory`),
...
function field(value: unknown, name: string): FieldSchema {
  const input = strictObject(value, name, [
    'type',
    'enum',
    'properties',
    'required',
  ...
  if (input.diagnostics != null) {
    ...
    diagnostics = Object.fromEntries(
      Object.entries(values).map(([code, description]) => [
  ...
  if (kind === 'field') {
    ...
    if (hasSources) {
      ...
      sources = Object.fromEntries(
        entries.map(([key, item]) => [string(key, `${name}.sources key`), reference(item, `${name}.sources.${key}`)]),
      );
    }
    return {
      kind,
      id: string(current.id, `${name}.id`),
      fieldSchema: string(current.fieldSchema, `${name}.fieldSchema`),
      source: hasSource ? reference(current.source, `${name}.source`) : undefined,
      sources,
      condition: condition(current.condition, `${name}.condition`),
    };
  }
  if (kind === 'group') {
    ...
    if (current.bind != null) {
      ...
      bind = Object.fromEntries(
        Object.entries(values).map(([key, item]) => [
          string(key, `${name}.bind key`),
          reference(item, `${name}.bind.${key}`),
        ]),
      );
    }
    return {
      kind,
      id: string(current.id, `${name}.id`),
      label: string(current.label, `${name}.label`),
      bind,
      condition: condition(current.condition, `${name}.condition`),
      children: current.children.map((child, index) => node(child, `${name}.children[${index}]`)),
    };
  }
  if (kind === 'rest') {
    ...
    return {
      ...
      id: string(current.id, `${name}.id`),
      label: string(current.label, `${name}.label`),
      path: reference(current.path, `${name}.path`),
      hide: current.hide as string[] | undefined,
      condition: condition(current.condition, `${name}.condition`),
    };
  }
...
export function parseFieldSchemaBundle(value: unknown): FieldSchemaBundle {
  ...
  const input = strictObject(candidate, 'FieldSchemaBundle', [
    'kind',
    'formatVersion',
    'schemaId',
    'revision',
  ...
  const fields = Object.fromEntries(
    Object.entries(values).map(([id, definition]) => [
...
export function parseUISchema(
  ...
  const input = strictObject(value, 'UISchema', [
    'kind',
    'formatVersion',
    'schemaId',
    'revision',
