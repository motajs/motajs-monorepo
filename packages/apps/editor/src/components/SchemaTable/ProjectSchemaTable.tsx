...
const SchemaErrorRepair: FC<{
  ...
  return (
    <div className="schemaProjectError" data-test-id="schema-project-error" role="alert">
      ...
      {hasOverride ? (
        <Space>
          <Button size="small" onClick={() => void openRaw()}>打开原始 JSON</Button>
          <Button size="small" danger onClick={() => void deleteSchemaOverride(definition, layer)}>删除 override</Button>
        </Space>
