...
export function fieldShapeIssues(schema: FieldSchema, slot: RawSlot<unknown>): FieldShapeIssue[] {
  ...
  if (
    (schema.editor.kind === 'stringList' || schema.editor.kind === 'orderedStringList') &&
  ...
  if (schema.editor.kind === 'equipmentSlots' && Array.isArray(slot.value)) {
    slot.value.forEach((item, index) => {
      if (item != null && typeof item !== 'string')
        issues.push({ path: [index], value: item, message: 'Expected string | null' });
    });
  }
  if (
    schema.editor.kind === 'itemCountRecord' &&
    slot.value &&
    ...
    for (const [key, count] of Object.entries(slot.value as Record<string, unknown>)) {
      if (typeof count !== 'number' || !Number.isFinite(count))
        issues.push({ path: [key], value: count, message: 'Expected finite number' });
    }
  }
  return issues;
}
...
export function fieldExpectedDescription(schema: FieldSchema): string {
  const types =
    schema.type == null ? 'any JSON value' : Array.isArray(schema.type) ? schema.type.join(' | ') : schema.type;
...
export function resolveBoundReference(
  reference: DataReference,
  bindings: ReferenceBindings = new Map(),
): ReferencedPath {
...
export function collectReferencedPaths(schema: UISchema): ReferencedPath[] {
  ...
  const visit = (nodes: UINode[], bindings: ReferenceBindings): void =>
    nodes.forEach((node) => {
      if (node.kind === 'field') {
        if (node.source) result.push(resolveBoundReference(node.source, bindings));
        if (node.sources)
          result.push(...Object.values(node.sources).map((reference) => resolveBoundReference(reference, bindings)));
      }
...
export function collectRestEntries(
  ...
  const visit = (object: Record<string, unknown>, prefix: string[]): RestEntry[] =>
