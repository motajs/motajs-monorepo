...
import { Button, Checkbox, Drawer, Dropdown, Input, Modal, Select, Space, Tooltip, type MenuProps } from 'antd';
...
import { loadSchemaOverrideDraft, saveSchemaOverride, saveSchemaSources } from './schemaOverrideCommands';
import { type BuiltinSchemaDefinition, type ProjectSchemaResolution, useProjectSchema } from './projectSchema';
import {
  createUniqueNodeId,
  findNodeLocation,
  insertNode,
...
import { selectSchemaCustomizationNode, useSchemaCustomizationState } from './schemaCustomizationState';
...
const NodeDrawer: FC<NodeDrawerProps> = ({ definition, resolution, selectedNodeId, onSelect }) => {
  ...
  const fieldOwner =
    sourceNode?.kind === 'field' ? (resolution.fieldOwners.get(sourceNode.fieldSchema) ?? definition) : definition;
  const ownerResolution = useProjectSchema(fieldOwner);
  const sourceField =
    sourceNode?.kind === 'field' && ownerResolution.status === 'ready'
  ...
  const builtinField = sourceNode?.kind === 'field' ? fieldOwner.fieldSource.fields[sourceNode.fieldSchema] : undefined;
  ...
  const restoreFields = async () => {
    ...
    await saveSchemaSources(
      definition,
      { field: { ...ownerResolution.fieldSource, fields } },
      `还原 ${restoreFieldIds.length} 个 Field`,
      fieldOwner,
    );
          ...
          <>
            <section>
              ...
              {nodeDraft.kind === 'field' ? (
                <>
                  <label className="schemaCustomizationControl">
                    Field Schema
                    <Select
                      showSearch
                      value={nodeDraft.fieldSchema}
                      options={[...resolution.fieldSchemas.entries()].map(([id, field]) => ({
                        value: id,
                        label: `${id} · ${field.title}`,
                      }))}
                      onChange={(fieldSchema) => {
                        setNodeDraft({ ...nodeDraft, fieldSchema });
                        if (sourceNode.kind !== 'field' || fieldSchema !== sourceNode.fieldSchema)
                          setFieldDraft(undefined);
                      }}
                    />
                  </label>
                  <JsonFragmentEditor
                    value={nodeDraft.source ?? nodeDraft.sources}
                    label="数据来源"
                    onChange={(value) => {
              ...
              {nodeDraft.kind === 'rest' ? (
                <>
                  <label className="schemaCustomizationControl">
                    Rest path
                    <Input
                      value={nodeDraft.path.ref}
                      onChange={(event) => setNodeDraft({ ...nodeDraft, path: { ref: event.target.value } })}
                    />
                  </label>
                  <label className="schemaCustomizationControl">
                    Hide dotted subtrees
                    <Input
                      value={(nodeDraft.hide ?? []).join(', ')}
                      onChange={(event) =>
                        setNodeDraft({
                          ...nodeDraft,
              ...
              <JsonFragmentEditor
                value={nodeDraft}
                label="当前节点 JSON"
                onChange={(value) => setNodeDraft(value as UINode)}
              />
              <Space wrap>
                {builtinNode ? (
                  <Button
                    ...
                    onClick={() =>
                      impactConfirm('还原节点属性？', '保留当前父级和顺序，只恢复内置节点属性。', async () =>
                        saveSchemaSources(
                          definition,
                          { ui: restoreNodeProperties(resolution.uiSource, definition.uiSource, sourceNode.id) },
                          `还原节点属性 ${sourceNode.id}`,
                        ),
                      )
                    }
                  >
                ...
                {builtinNode ? (
                  <Button
                    onClick={() =>
                      impactConfirm('还原节点位置？', '节点将回到内置父级和相对顺序，当前属性保持不变。', async () =>
                        saveSchemaSources(
                          definition,
                          { ui: restoreNodePosition(resolution.uiSource, definition.uiSource, sourceNode.id) },
                          `还原节点位置 ${sourceNode.id}`,
                        ),
                      )
                    }
                  >
                    ...
                    onClick={() =>
                      impactConfirm(
                        ...
                        async () =>
                          saveSchemaSources(
                            definition,
                            { ui: restoreGroupSubtree(resolution.uiSource, definition.uiSource, sourceNode.id) },
                            `还原 Group ${sourceNode.id}`,
                          ),
                      )
                    }
                  >
                  ...
                  <>
                    <label className="schemaCustomizationControl">
                      ...
                      <Input
                        ...
                        onChange={(event) => setFieldDraft({ ...fieldDraft, title: event.target.value })}
                      />
                    </label>
                    <label className="schemaCustomizationControl">
                      说明
                      <Input.TextArea
                        ...
                        onChange={(event) =>
                          setFieldDraft({ ...fieldDraft, description: event.target.value || undefined })
                        }
                      />
                    </label>
                    <JsonFragmentEditor
                      value={fieldDraft}
                      label="当前 Field JSON"
                      onChange={(value) => setFieldDraft(value as FieldSchema)}
                    />
                    <Space wrap>
                      {builtinField ? (
                        <Button
                                ...
                                saveSchemaSources(
                                  definition,
                                  {
                                    field: {
                                      ...ownerResolution.fieldSource,
                                      fields: {
                                        ...ownerResolution.fieldSource.fields,
                                        [sourceNode.fieldSchema]: structuredClone(builtinField),
                                      },
                                    },
                                  },
                                  `还原 Field ${sourceNode.fieldSchema}`,
                                  fieldOwner,
                                ),
                            )
                          }
                      ...
                      <Button icon={<Code2 size={14} />} onClick={() => void openWholeLayer('field')}>
                        打开完整 Field Bundle
                      </Button>
                      <Button
                        onClick={() =>
...
export interface SchemaCustomizationEditorProps extends Omit<
  SchemaTableProps,
  'fieldSchemas' | 'uiSchema' | 'customization'
> {
  definition: BuiltinSchemaDefinition;
  resolution: ReadyResolution;
}
...
export const SchemaCustomizationEditor: FC<SchemaCustomizationEditorProps> = ({ definition, resolution, ...props }) => {
  ...
  const missingBuiltinNodes = listNodeLocations(definition.uiSource).filter(
    (location) => !findNodeLocation(resolution.uiSource, location.node.id),
  );
  ...
  const addNode = async () => {
    ...
    if (addDraft.kind === 'field') {
      ...
      node = {
        kind: 'group',
        id: createUniqueNodeId(resolution.uiSource, 'custom-group'),
        label: addDraft.label || '新分组',
        children: [],
      };
    } else {
      node = {
        kind: 'rest',
        id: createUniqueNodeId(resolution.uiSource, 'custom-rest'),
        label: addDraft.label || '其他字段',
        path: { ref: addDraft.path || defaultRootRef },
      };
    }
    ...
    Modal.confirm({
      ...
      content:
        location.node.kind === 'group'
          ? 'Group 的整个子树都会从 Table 中删除；Field 定义不会随节点删除。'
          : 'Field 定义不会随节点删除。',
      okText: '删除',
      okButtonProps: { danger: true },
      cancelText: '取消',
    ...
    <div className="schemaCustomizationMode" data-test-id="schema-customization-mode">
      <div className="schemaCustomizationToolbar" data-test-id="schema-customization-toolbar">
        ...
        <Button
          ...
          danger
          icon={<Trash2 size={14} />}
          onClick={() => (selected ? confirmDeleteNode(selected.node.id) : undefined)}
        >
          删除节点
        </Button>
        <Button
          size="small"
          disabled={missingBuiltinNodes.length === 0}
          icon={<RotateCcw size={14} />}
          onClick={() => setRecoverOpen(true)}
        >
          找回内置节点
        </Button>
        <Button size="small" icon={<Code2 size={14} />} onClick={() => void openTableSource()}>
          完整 Table 源码
        </Button>
        <Tooltip title="删除当前 Table override，Field Bundle 自定义保持不变">
          <Button
            size="small"
            onClick={() =>
              impactConfirm('还原整张 Table？', '布局、顺序和全部自定义节点将恢复为当前编辑器内置版本。', async () =>
                saveUi(definition.uiSource, '还原整张 Table'),
              )
            }
          >
            还原整张 Table
      ...
      <SchemaTable
        ...
        customization={{
          ...
          onMove: (source, target, placement) => {
            void saveUi(moveNode(resolution.uiSource, source, target, placement), `移动 Schema 节点 ${source}`).catch(
              notifyError,
            );
          },
      ...
      >
        {addDraft?.kind === 'field' ? (
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            />
            <Input
              placeholder="source，例如 floor:title"
              value={addDraft.source}
              onChange={(event) => setAddDraft({ ...addDraft, source: event.target.value })}
            />
          </Space>
        ) : null}
        {addDraft?.kind === 'group' ? (
          <Input
            ...
            onChange={(event) => setAddDraft({ ...addDraft, label: event.target.value })}
          />
        ) : null}
        {addDraft?.kind === 'rest' ? (
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            />
            <Input
              placeholder={`path，例如 ${defaultRootRef}`}
              value={addDraft.path}
              onChange={(event) => setAddDraft({ ...addDraft, path: event.target.value })}
            />
          </Space>
        ) : null}
      </Modal>
      <Modal
        ...
        onOk={() => {
          if (!recoverId) return;
          void saveUi(
            recoverBuiltinNode(resolution.uiSource, definition.uiSource, recoverId),
            `找回内置节点 ${recoverId}`,
          )
            .then(() => {
      ...
      >
        <Select
          ...
          placeholder="选择内置节点"
          value={recoverId}
          options={missingBuiltinNodes.map(({ node }) => ({
            value: node.id,
            label: `${node.id} · ${node.kind === 'field' ? node.fieldSchema : node.label}`,
          }))}
