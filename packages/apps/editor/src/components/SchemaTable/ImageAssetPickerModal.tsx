...
import { type FC, type PointerEvent as ReactPointerEvent, useEffect, useMemo, useRef, useState } from 'react';
...
const ImageCropPreview: FC<{
  ...
  const maxHeight = 390;
  const scale =
    natural.width > 0 && natural.height > 0 ? Math.min(maxWidth / natural.width, maxHeight / natural.height) : 1;
              ...
              <InputNumber
                ...
                onChange={(value) => {
                  ...
                  if (key === 'x') onCropChange({ ...crop, x: Math.max(0, Math.min(natural.width - 1, numeric)) });
                    ...
                    onCropChange({ ...crop, y: Math.max(0, Math.min(natural.height - 1, numeric)) });
                  else if (key === 'width')
                    onCropChange({ ...crop, width: Math.max(1, Math.min(natural.width - crop.x, numeric)) });
                  else onCropChange({ ...crop, height: Math.max(1, Math.min(natural.height - crop.y, numeric)) });
                }}
              />
            </label>
          ))}
          <button type="button" onClick={() => onCropChange(undefined)}>
            使用完整图片
          </button>
        </div>
      ) : null}
...
export const ImageAssetPickerModal: FC<ImageAssetPickerModalProps> = ({
  ...
  const entries = useMemo(
    () => catalog.entries.filter((entry) => (includeLogical || entry.kind === 'physical') && (accept?.(entry) ?? true)),
    ...
    >
      <div className="floorImageAssetPicker" data-test-id="image-asset-picker">
        <div className="floorImageAssetBrowser">
          <Input allowClear placeholder="搜索图片" value={query} onChange={(event) => setQuery(event.target.value)} />
