...
import { createSchemaOverrideDraft, validateAndNormalizeSchemaOverride } from '../schemaOverrideCommands';
...
describe('project SchemaTable overrides', () => {
  ...
  it('surfaces a broken override instead of silently falling back', () => {
    const malformed = resolveProjectSchema(floorSchemaDefinition, { status: 'loaded', value: '{ nope' }, missing);
    expect(malformed).toMatchObject({ status: 'error', layer: 'field' });

    const unknownField = resolveProjectSchema(floorSchemaDefinition, missing, {
      status: 'loaded',
  ...
  it('resolves Table fields through the global cross-Bundle registry', () => {
    ...
    const result = resolveProjectSchema(floorSchemaDefinition, missing, {
      status: 'loaded',
      value: JSON.stringify(uiSource),
    });
    expect(result.status).toBe('ready');
    if (result.status === 'ready')
      expect(result.fieldOwners.get('tower.version')?.fieldBundle.schemaId).toBe('tower.fields');
  });
  ...
  it('forks the complete built-in source and records its origin', async () => {
    ...
    expect(schemaOverridePath(floorSchemaDefinition, 'ui')).toBe('.metaphysics/schemas/ui/floor-properties.json');
  });

  it('normalizes metadata but rejects a schemaId from another table', async () => {
    const normalized = JSON.parse(
      await validateAndNormalizeSchemaOverride(
        floorSchemaDefinition,
        'field',
