...
describe('SchemaTable normalizers', () => {
  ...
  it('round-trips nullable scalar/list BGM values', () => {
    ...
    expect(nullableScalarOrListNormalizer.toEdit({ present: true, value: ['a.mp3', 'b.ogg'] })).toEqual([
      'a.mp3',
      'b.ogg',
    ]);
    expect(nullableScalarOrListNormalizer.toRaw([], { present: false })).toEqual({ present: true, value: null });
    expect(nullableScalarOrListNormalizer.toRaw(['a.mp3'], { present: true, value: null })).toEqual({
      present: true,
      value: 'a.mp3',
    });
    expect(nullableScalarOrListNormalizer.toRaw(['a.mp3', 'b.ogg'], { present: false })).toEqual({
      present: true,
      value: ['a.mp3', 'b.ogg'],
    });
  });
  ...
  it('normalizes optional event lists to missing when empty', () => {
    ...
    expect(optionalEventListNormalizer.toRaw(null, { present: true, value: [{ type: 'comment' }] })).toEqual({
      present: false,
    });
    expect(optionalEventListNormalizer.toRaw([], { present: true, value: [{ type: 'comment' }] })).toEqual({
      present: false,
    });
    expect(optionalEventListNormalizer.toRaw([{ type: 'comment' }], { present: false })).toEqual({
      present: true,
      value: [{ type: 'comment' }],
    });
  ...
  it('preserves configured event objects while keeping the compact array form', () => {
    ...
    expect(() =>
      optionalEventNormalizer.toEdit({
        present: true,
  ...
  it('normalizes auto-event page arrays and records into a sorted editable list', () => {
    expect(
      autoEventPagesNormalizer.toEdit({
        present: true,
    ...
    expect(
      autoEventPagesNormalizer.toRaw(
        [
          { id: 0, value: null },
          { id: 1, value: { condition: 'flag:door', data: [] } },
        ],
