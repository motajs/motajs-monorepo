...
function parseFieldSchemaBundle(value: Record<string, unknown>) {
  ...
  const records = Object.fromEntries(
    (value.fields as Array<Record<string, unknown>>).map((item) => {
...
describe('SchemaTable schema parser', () => {
  ...
  it('accepts a combine editor with suggestion and number inputs', () => {
    ...
    expect(parsed.fields['test.weather'].editor).toMatchObject({
      kind: 'combine',
      inputs: [
        { key: 'type', editor: { kind: 'suggestion' } },
        { key: 'level', editor: { kind: 'number' } },
      ],
    });
  });
  ...
  it('parses item applicability conditions without executable schema strings', () => {
    ...
    expect(use?.kind === 'group' ? use.children.map((item) => item.condition?.otherwise ?? 'always') : []).toEqual([
      'always',
      'inactive',
      'inactive',
      'inactive',
  ...
  it('parses enemy registry choices and named applicability calls', () => {
    ...
    expect(
      specials?.kind === 'group'
        ? specials.children.find((item) => item.kind === 'field' && item.fieldSchema === 'enemy.n')?.condition
        : undefined,
    ).toEqual({
      when: {
  ...
  it('parses the floor dimensions as one combined field', () => {
    ...
    expect(
      basic?.kind === 'group'
        ? basic.children.find((item) => item.kind === 'field' && item.fieldSchema === 'floor.size')
        : undefined,
    ).toMatchObject({
  ...
  it('requires exactly one single or named source binding for a field', () => {
    ...
    expect(() => parseUISchema({ schemaId: 'none', version: 1, nodes: [base] }, fields)).toThrow(
  ...
  it.each(['list', 'record', 'action'])('rejects unsupported %s nodes', (kind) => {
    ...
    ).toThrow(`Unsupported UI node kind: ${kind}`);
  });
  ...
  it('rejects unknown fields and malformed expressions', () => {
    ...
    ).toThrow('Unknown Field Schema ID');
    expect(() =>
      parseUISchema(
        {
          schemaId: 'test.ui',
  ...
  it('rejects the verification array format and slash references', () => {
    ...
    ).toThrow('unsupported v1 reference syntax');
  });
  ...
  it('isolates duplicate global Field IDs as ambiguous', () => {
    ...
    ).toThrow('Ambiguous Field Schema ID');
  });
});
