...
describe("SchemaTable Rest", () => {
  it("recursively collects unconsumed paths, hides literal subtrees, and treats arrays atomically", () => {
    ...
    }, { root: "floor", path: [] }, collectReferencedPaths(schema), ["map", "nested.hidden"]);
    expect(entries).toEqual([
      { path: ["nested"], value: { used: 2, second: 5, extra: 3, hidden: { value: 4 } }, children: [{ path: ["nested", "extra"], value: 3 }] },
