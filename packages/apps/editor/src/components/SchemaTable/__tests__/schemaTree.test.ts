...
describe('SchemaTable structure editing', () => {
  it('moves nodes across Group/root boundaries', () => {
    ...
    expect(nested.nodes[0].kind === 'group' ? nested.nodes[0].children.map((node) => node.id) : []).toEqual([
      'b',
      'rest',
    ]);
  });
  ...
  it('restores properties independently from position', () => {
    ...
    expect(position.nodes[0].kind === 'group' ? position.nodes[0].children.map((node) => node.id) : []).toEqual([
      'a',
      'b',
    ]);
  });
