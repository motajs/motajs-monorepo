...
describe("SchemaTable component", () => {
  ...
  it("keeps Group order and applies all four condition presentations", () => {
    ...
    render(
      <SchemaTable
        fieldSchemas={fields}
        uiSchema={schema}
        scope={{ roots: { floor: new ObjectReferenceRoot("floor", () => data, async () => undefined) } }}
      />,
    );
  ...
  it("edits enum choices and checkbox sets while keeping unknown selected values visible", async () => {
    const fields = new Map<string, FieldSchema>([
      ["trigger", {
        ...
        editor: {
          kind: "select",
          options: [{ value: null, label: "Unset" }, { value: "openDoor", label: "Open door" }],
        },
      }],
      ["directions", {
        $id: "directions",
    ...
    await waitFor(() => expect(writes).toHaveBeenCalledWith(
      ["directions"],
      { present: true, value: ["up", "custom", "down"] },
    ));
  });
  ...
  it("edits a combined passability field through one batched source update", async () => {
    ...
    const fields = new Map<string, FieldSchema>([["passability", {
      $id: "passability",
      type: "object",
      title: "Passability",
    ...
    await waitFor(() => expect(writeBatch).toHaveBeenCalledWith([{
      path: ["cannotIn"],
  ...
  it("selects the initial floor and coordinates through the map point picker", async () => {
    ...
    const fields = new Map<string, FieldSchema>([["position", {
      $id: "position",
      type: "object",
    ...
    await waitFor(() => expect(modalMocks.selectPoint).toHaveBeenCalledWith(expect.objectContaining({
      floorId: "sample0",
    ...
    await waitFor(() => expect(writeBatch).toHaveBeenCalledWith(expect.arrayContaining([
  ...
  it("synchronizes long JSON values without relying on a truncated remount key", () => {
    const fields = new Map<string, FieldSchema>([["payload", {
      $id: "payload", type: "object", title: "Payload", editor: { kind: "json" },
    }]]);
    const schema: UISchema = {
      schemaId: "long-json", version: 1,
    ...
    const view = render(<SchemaTable
      fieldSchemas={fields}
      uiSchema={schema}
      scope={{ roots: { floor: new ObjectReferenceRoot("floor", () => first, async () => undefined) } }}
    />);
    expect((screen.getByRole("textbox") as HTMLTextAreaElement).value).toContain(`${"a".repeat(100)}1`);
    view.rerender(<SchemaTable
      fieldSchemas={fields}
      uiSchema={schema}
      scope={{ roots: { floor: new ObjectReferenceRoot("floor", () => second, async () => undefined) } }}
    />);
  ...
  it("derives clear support from nullable field type", async () => {
    const data = { value: 5 };
    const writes = vi.fn(async () => undefined);
    const fields = new Map<string, FieldSchema>([["value", {
      $id: "value", type: ["number", "null"], title: "Value", editor: { kind: "number" },
    }]]);
    const schema: UISchema = {
      schemaId: "nullable", version: 1,
  ...
  it("rerenders a condition when an asynchronous registry source becomes ready", async () => {
    ...
    const schema: UISchema = {
      schemaId: "async-condition", version: 1,
      nodes: [{
        kind: "field", fieldSchema: "value", source: { ref: "floor:value" },
        condition: { when: { ref: "registry:enabled" }, otherwise: "hidden" },
      }],
    };
    render(<SchemaTable fieldSchemas={new Map([["value", field("value", "Value")]])} uiSchema={schema} scope={{ roots }} />);
    expect(screen.getByLabelText("loading")).toBeTruthy();
  ...
  it("locates diagnostics on the referenced field and repairs mismatched raw JSON with direct set", async () => {
    ...
    const schema: UISchema = {
      ...
      nodes: [{
        ...
        label: "Main",
        children: [
          { kind: "field", fieldSchema: "title", source: { ref: "floor:title" } },
        ],
      }],
    };
    render(
  ...
  it("repairs an invalid collection item without replacing the whole Field JSON", async () => {
    ...
    const fields = new Map<string, FieldSchema>([["values", {
      type: "array",
      items: { type: "string" },
      title: "Values",
      editor: { kind: "json" },
    ...
    render(<SchemaTable
      fieldSchemas={fields}
      uiSchema={schema}
      scope={{ roots: { floor: new ObjectReferenceRoot("floor", () => data, async (path, slot) => {
        if (slot.present) data[path[0]] = slot.value;
      }) } }}
    />);
  ...
  it("provides recursive Rest JSON update, delete and add operations", async () => {
    ...
    const schema: UISchema = {
      schemaId: "rest",
      version: 1,
      nodes: [{ kind: "field", fieldSchema: "known", source: { ref: "floor:known" } }, {
        kind: "rest",
        id: "rest",
        label: "Other",
        path: { ref: "floor:" },
    ...
    await waitFor(() => expect(writes).toHaveBeenCalledWith(["created.with.dot"], { present: true, value: { ok: true } }));
  });
});
