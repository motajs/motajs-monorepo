...
describe('openExternalEditor', () => {
  it('opens structured events through EventEditor', () => {
    ...
    openExternalEditor("['startCanvas']", 'event', { _event: 'event' } as FieldConfig, () => initialValue, setValue, {
      eventEditor: { open },
    });
  ...
  it('round-trips function text through CodeEditor without JSON quoting', () => {
    ...
    openExternalEditor("['init']", 'textarea', {} as FieldConfig, () => 'function init () { return true; }', setValue, {
      codeEditor: { open },
    });
  ...
  it('uses modern point, material and checkbox capabilities', async () => {
    ...
    openExternalEditor("['upFloor']", 'point', {} as FieldConfig, () => [2, 3], setValue, { selectPoint });
