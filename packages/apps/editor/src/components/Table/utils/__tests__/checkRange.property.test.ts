 ...
 */
const numericRangeConfigArb = fc
  .tuple(fc.integer({ min: -1000, max: 0 }), fc.integer({ min: 1, max: 1000 }))
 ...
 * 生成 _select 配置
 */
const selectConfigArb = fc
  .array(fc.oneof(fc.integer(), fc.string()), { minLength: 1, maxLength: 10 })
...
describe('checkRange 属性测试', () => {
  describe('Property 1: Unified checkRange Validation', () => {
    it('对于任意 _range 配置，checkRange 返回 false 时值应被拒绝', () => {
      fc.assert(
        fc.property(numericRangeConfigArb, fc.integer(), (config, value) => {
    ...
    it('对于任意 _select 配置，只有在 values 中的值才应通过验证', () => {
      fc.assert(
        fc.property(selectConfigArb, fc.oneof(fc.integer(), fc.string()), (config, value) => {
          const result = checkRange(config, value);
    ...
    it('对于无验证规则的配置，任意值都应通过验证', () => {
      fc.assert(
        fc.property(noValidationConfigArb, fc.jsonValue(), (config, value) => {
          const result = checkRange(config, value);
...
describe('checkRange 属性测试 - Property 2', () => {
  describe('Property 2: Valid Values Are Saved', () => {
     ...
     */
    const createValueChangeHandler = (config: FieldConfig, onValueChange: (value: unknown) => void) => {
    ...
    it('对于任意通过 _range 验证的值，应调用 onChange 回调', () => {
      fc.assert(
        fc.property(fc.tuple(fc.integer({ min: -1000, max: 0 }), fc.integer({ min: 1, max: 1000 })), (bounds) => {
    ...
    it('对于任意通过 _select 验证的值，应调用 onChange 回调', () => {
      fc.assert(
        fc.property(selectConfigArb, (config) => {
