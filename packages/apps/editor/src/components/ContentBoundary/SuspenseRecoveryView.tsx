...
import { LoadingRecovery, NotFoundRecovery, ParseErrorRecovery, IOErrorRecovery } from './RecoveryUI';
 ...
 * 用于用户点击 retry 后的等待状态
 */
export const SuspenseRecoveryView: FC<SuspenseRecoveryViewProps> = ({ handler, onClear, recoveryUI, loadingUI }) => {
  ...
  switch (content.status) {
    ...
    case 'not-found':
      return <NotFoundRecovery path={path} onRetry={() => handler.refetch()} />;

    case 'error': {
      // 区分 parse-error 和 io-error：有 rawContent 说明文件读取成功但解析失败
      const raw =
        'raw' in handler && typeof handler.raw === 'function'
          ? handler.raw()
      ...
      if (rawContent !== undefined) {
        return <ParseErrorRecovery error={content.error} onRetry={() => handler.refetch()} />;
      }
      return <IOErrorRecovery error={content.error} onRetry={() => handler.refetch()} />;
    }
