...
export function RuntimeProvider({ children }: { children?: ReactNode }) {
            ...
            void (async () => {
              try {
                if (message.binary) {
                  ...
                  channel.port1.postMessage({ type: "resource-response", id: message.id, ok: true, revision: resource.revision, bytes } satisfies HostMessage, [bytes]);
                } else {
                  const resource = await gatewayRef.current.readText(message.path);
                  channel.port1.postMessage({ type: "resource-response", id: message.id, ok: true, revision: resource.revision, text: resource.text } satisfies HostMessage);
                }
              } catch (error) {
                channel.port1.postMessage({ type: "resource-response", id: message.id, ok: false, error: error instanceof Error ? error.message : String(error) } satisfies HostMessage);
              }
            })();
            return;
          }
  ...
  const createLease = useCallback((size: { width: number; height: number }): RuntimeSurfaceLease => {
    const iframe = iframeRef.current!;
    let stopPositioning: (() => void) | null = null;
    const lease: RuntimeSurfaceLease = {
      id: crypto.randomUUID(), width: size.width, height: size.height,
      attach(container) {
        const host = hiddenRef.current;
        if (!host) return;
  ...
  const queuePreview = useCallback((request: HostRequestPayload): Promise<RuntimeSurfaceLease> => {
    ...
    previewQueueRef.current = operation.then(() => undefined, () => undefined);
    return operation;
  }, [createLease, send]);
  ...
  return <RuntimeContext.Provider value={capability}>
    {children}
    <div
      ref={hiddenRef}
      ...
      style={{ position: "fixed", left: -10000, top: 0, zIndex: 100000, width: 416, height: 416, overflow: "hidden", pointerEvents: "none", visibility: "hidden" }}
    >
      <iframe ref={iframeRef} data-test-id="runtime-iframe" title="Mota runtime" sandbox="allow-scripts allow-same-origin" style={{ border: 0, width: 416, height: 416 }} />
    </div>
