      ...
      props.items.flatMap((item) => {
        ...
        if (item.idnum < 10000) {
          ...
          return [
            {
              ...item,
              materialPath: block.editorDisplay?.type === 'image' ? block.materialPath : item.materialPath,
              x: block.editorDisplay?.x ?? item.x,
              y: block.editorDisplay?.y ?? item.y,
            },
          ];
        }
        return tilesetCatalog.entries.some(
          (entry) => item.idnum >= entry.startIdnum && item.idnum < entry.startIdnum + entry.columns * entry.rows,
        )
          ? [item]
          : [];
      }),
...
function isTextEditorTarget(target: EventTarget | null): boolean {
  const hasVisibleEditorOverlay = ['#uieventDiv', "[data-test-id='event-editor']", "[data-test-id='code-editor']"].some(
    (selector) => {
...
const MapEditorInner: FC = () => {
  ...
  const discoveredFloorId =
    towerContent.status === 'loaded'
      ? (() => {
          const floorIds = Array.isArray(towerContent.value.main.floorIds) ? towerContent.value.main.floorIds : [];
        ...
        restore: (viewport) => {
          flushSync(() => {
            ...
            if (viewport.locSelection) {
              setCurrentLocPos(cloneDeep(viewport.locSelection.pos), viewport.locSelection.floorId);
            } else {
  ...
  useEffect(
    () =>
      subscribeNotifications(({ level, message }) => {
  ...
  const handleFloorStep = useCallback(
    (delta: number) => {
  ...
  useEffect(() => {
    const handleMapShortcut = (event: KeyboardEvent) => {
      ...
      if (commandKey && ['c', 'x', 'v'].includes(key) && state.selectedBlock === undefined) {
        ...
        void mapCommands
          .clearArea(floorId, state.layerMod, rect)
          .then((result) => notifyCommandResult(result, '地图区域已剪切'));
        return;
      }
      ...
      if (event.key === 'Delete' && state.selectedBlock === undefined) {
        event.preventDefault();
        void mapCommands
          .clearArea(floorId, state.layerMod, selectionRect(state.selectedArea, state.pos))
      ...
      <div ref={midRef} id="mid" data-test-id="map-editor-mid">
        ...
        {floorId ? (
          <ContentBoundary key={`tools:${floorId}`} loadingUI={<div className="tools" />}>
            <ToolBar floorId={floorId} tipMessage={tipMessage} tipClass={tipClass} onFloorChange={handleFloorChange} />
          </ContentBoundary>
        ) : (
          <div className="tools" />
        )}
        ...
        <ContentBoundary loadingUI={<></>}>
          <ValidatedRecentlyUsedPanel
            ...
            onToggleTop={(item, istop) =>
              setLastUsed(lastUsed.map((one) => (one.idnum === item.idnum ? { ...one, istop: istop ? 1 : 0 } : one)))
            }
            onClear={() => setLastUsed([])}
          />
        </ContentBoundary>
