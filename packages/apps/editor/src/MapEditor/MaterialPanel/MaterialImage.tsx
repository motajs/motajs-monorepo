...
export const MaterialImage: FC<MaterialImageProps> = ({
  ...
  const imageGridSize = useMemo<GridPOD>(() => image
    ? [Math.floor(image.width / grid[0]), Math.floor(image.height / grid[1])]
    : [0, 0], [grid, image]);
  ...
  const handleContextMenu = useCallback((event: MouseEvent<HTMLElement>) => {
    ...
    if (
      width <= 0 || height <= 0
      || start[0] + width > imageGridSize[0]
      || start[1] + height > imageGridSize[1]
    ) {
