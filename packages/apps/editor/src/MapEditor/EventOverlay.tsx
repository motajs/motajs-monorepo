 ...
 * 获取位置的事件颜色列表
 */
function getEventColors(loc: string, floorData: Record<string, unknown>): string[] {
  const colors: string[] = [];
  ...
  if ((floorData.beforeBattle as Record<string, unknown> | undefined)?.[loc]) {
    colors.push(EVENT_COLORS.beforeBattle);
  }

  if ((floorData.afterBattle as Record<string, unknown> | undefined)?.[loc]) {
    colors.push(EVENT_COLORS.afterBattle);
  }

  if ((floorData.changeFloor as Record<string, unknown> | undefined)?.[loc]) {
    colors.push(EVENT_COLORS.changeFloor);
  }

  if ((floorData.afterGetItem as Record<string, unknown> | undefined)?.[loc]) {
    colors.push(EVENT_COLORS.afterGetItem);
  }

  if ((floorData.afterOpenDoor as Record<string, unknown> | undefined)?.[loc]) {
...
function drawMovableOverlay(
  ...
  const directions: Array<{
  ...
  }> = [
    {
      ...
      dx: -1,
      dy: 0,
      line: (x, y, size, full) => [x, y + size * (full ? 0.2 : 0.33), x, y + size * (full ? 0.8 : 0.67)],
    },
    {
      direction: 'right',
      dx: 1,
      dy: 0,
      line: (x, y, size, full) => [x + size, y + size * (full ? 0.2 : 0.33), x + size, y + size * (full ? 0.8 : 0.67)],
    },
    {
      direction: 'up',
      dx: 0,
      dy: -1,
      line: (x, y, size, full) => [x + size * (full ? 0.2 : 0.33), y, x + size * (full ? 0.8 : 0.67), y],
    },
    {
      direction: 'down',
      dx: 0,
      dy: 1,
      line: (x, y, size, full) => [x + size * (full ? 0.2 : 0.33), y + size, x + size * (full ? 0.8 : 0.67), y + size],
    },
  ];
  const drawOneWayArrow = (direction: MapDirection, x: number, y: number, size: number) => {
...
export const EventOverlay: FC<EventOverlayProps> = ({ floorId }) => {
  ...
  const { pos, bigmap, bigmapInfo, viewportOffset, showMovable, bindSpecialDoor, selectedArea } = state;
  ...
  const drawEventBlock = useCallback(() => {
    ...
    // 获取出生点信息
    const firstData = tower.firstData as { floorId: string; hero: { loc: { x: number; y: number } } } | undefined;
    ...
    for (let i = 0; i < GRID_SIZE; i++) {
      for (let j = 0; j < GRID_SIZE; j++) {
        ...
        // 绘制出生点标记 "S"
        if (firstData && floorId === firstData.floorId && loc === `${firstData.hero.loc.x},${firstData.hero.loc.y}`) {
        ...
        for (let k = 0; k < colors.length; k++) {
          ...
          ctx.fillRect(TILE_SIZE * i + 8 * k, TILE_SIZE * j + TILE_SIZE - 8, 8, 8);
        }
        ...
        if (enemyIndex >= 0) {
          ...
          ctx.strokeStyle = '#000000';
          ctx.lineWidth = 1;
          ctx.strokeText(String(enemyIndex + 1), TILE_SIZE * i + 28, TILE_SIZE * j + 15);
          ctx.fillText(String(enemyIndex + 1), TILE_SIZE * i + 28, TILE_SIZE * j + 15);
        }
        ...
        const flyPoint = (floor as Record<string, unknown>).flyPoint as [number, number] | undefined;
  ...
  }, [canvas, floor, tower, floorId, bigmap, bigmapInfo, viewportOffset, showMovable, bindSpecialDoor, passability]);
  ...
  const drawPosSelection = useCallback(() => {
    ...
    if (bigmap) {
      ...
      const psize = size / 8;
      ctx.lineWidth = psize;
      ctx.strokeRect(left + pos[0] * size + psize, top + pos[1] * size + psize, size - 2 * psize, size - 2 * psize);
    } else {
      const [offsetX, offsetY] = viewportOffset;
      ctx.lineWidth = 4;
      ctx.strokeRect(TILE_SIZE * pos[0] - offsetX + 4, TILE_SIZE * pos[1] - offsetY + 4, 24, 24);
    }
