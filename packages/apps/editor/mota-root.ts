...
const REQUIRED_PATHS = ['index.html', 'main.js', 'project/data.js', '_server/table/data.comment.js'] as const;

export function resolveMotaJsRoot(value = process.env.MOTA_JS_ROOT): string {
  const root = value ? path.resolve(process.cwd(), value) : path.resolve(import.meta.dirname, '../../external/mota-js');
