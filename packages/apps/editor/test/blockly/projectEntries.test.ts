...
import { dataToWorkspaceStateWithEntry, hasDedicatedEntryType, type ParseContext } from '@/blockly/parser';
...
function load(entryType: string, value: unknown, context: NonNullable<ParseContext['project']> | null = project) {
...
describe('project configuration Blockly entries', () => {
  ...
  it('round-trips sparse faceIds', () => {
    expect(roundTrip('faceIds', { down: 'npc0', up: 'npc3' })).toEqual({ down: 'npc0', up: 'npc3' });
  });
  ...
  it('covers every tableMeta event entry with a dedicated mapping', () => {
    ...
    const entryTypes = new Set(
      files.flatMap((file) =>
        [...readFileSync(path.join(MOTA_JS_ROOT, file), 'utf8').matchAll(/"_event"\s*:\s*"([^"]+)"/g)].map(
          (match) => match[1],
        ),
      ),
    );
