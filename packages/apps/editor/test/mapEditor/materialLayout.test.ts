...
describe("material layout", () => {
  it("keeps the complete sheet size but normalizes animation frames to the first column", () => {
    const layout = createMaterialLayout({
      sourceSize: [128, 320], cellSize: [32, 32], mode: "full", rowsPerColumn: 50,
    });
  ...
  it("folds 32px rows at the configured boundary", () => {
    const layout = createMaterialLayout({
      sourceSize: [128, 52 * 32], cellSize: [32, 32], mode: "folded", rowsPerColumn: 50,
    });
  ...
  it("uses the same row mapping for 48px materials", () => {
    const layout = createMaterialLayout({
      sourceSize: [128, 7 * 48], cellSize: [32, 48], mode: "folded", rowsPerColumn: 3,
    });
  ...
  it("collapses a single autotile to one representative cell", () => {
    const layout = createMaterialLayout({
      sourceSize: [96, 128], cellSize: [32, 32], mode: "folded", rowsPerColumn: 50, kind: "single",
    });
  ...
  it("never folds spatial tilesets", () => {
    const layout = createMaterialLayout({
      sourceSize: [320, 192], cellSize: [32, 32], mode: "folded", rowsPerColumn: 2, kind: "spatial",
    });
  ...
  it("supports the virtual terrain prefix used by clear and airwall", () => {
    const layout = createMaterialLayout({
      sourceSize: [32, 5 * 32], cellSize: [32, 32], mode: "folded", rowsPerColumn: 3,
    });
