      ...
      server.middlewares.use((req: IncomingMessage, res: ServerResponse, next: () => void) => {
        const handleRequest = async () => {
          ...
          if (pathname.startsWith('/__all_floors__.js')) {
            ...
            const tasks = all.map((v: string) => fs.readFile(path.join(motaRoot, `project/floors/${v}.js`))
          ...
          if (pathname.startsWith('/__all_animates__')) {
            ...
            const all = ids.split(',').filter(Boolean);
            const data: Record<string, Buffer> = {};
            const tasks = all.map((v: string) => fs.readFile(path.join(motaRoot, `project/animates/${v}.animate`))
              .then(content => { data[v] = content; })
              .catch(() => {}));
            await Promise.all(tasks);
            const result = all.map((v: string) => data[v]).join('@@@~~~###~~~@@@');
        ...
        if (req.method === 'POST') {
          ...
          if (pathname === '/writeMultiFiles') {
            ...
            const tasks = names.map((v, i) => fs.writeFile(resolveFsPath(v), values[i], 'base64')
