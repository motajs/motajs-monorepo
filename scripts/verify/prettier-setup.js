...
function checkConfigFiles() {
  check(fs.existsSync(PRETTIER_CONFIG_PATH), '缺少 .prettierrc.json —— Prettier 没有唯一的配置文件');
  check(fs.existsSync(PRETTIER_IGNORE_PATH), '缺少 .prettierignore —— 生成物/第三方/规划产物随时可能被改写');
...
function checkLockfile() {
  ...
  for (const name of NEW_DEV_DEPENDENCIES) {
    ...
    check(catalogEntry.test(lockfile), `pnpm-lock.yaml 的 catalog 未声明 ${name}`);
    check(packageEntry.test(lockfile), `pnpm-lock.yaml 未解析出 ${name} 的包条目`);
  }
}
